...
#ifdef __3DS__
    // On Nintendo 3DS, int32_t/uint32_t are typedefs of long types,
    // so plain int literals would otherwise match multiple constructors.
    Fixpoint(int i);

    Fixpoint(unsigned int i);
...
inline Fixpoint::Fixpoint(int i) {
    CLICK(constructor_int);
    val = ((int32_t)i) << SHIFTUP;
}

inline Fixpoint::Fixpoint(unsigned int i) {
    CLICK(constructor_uint);
    val = (int32_t)(((uint32_t)i) << SHIFTUP);
}

inline Fixpoint::Fixpoint(bool b) {
    CLICK(constructor_int);
    val = b ? SHIFTMULTIPLIER : 0;
}
#endif
...
#ifdef __3DS__
inline Fixpoint operator*(int i, Fixpoint const &fp) { return Fixpoint(i) * fp; }
inline Fixpoint operator*(unsigned int i, Fixpoint const &fp) { return Fixpoint(i) * fp; }
#endif
