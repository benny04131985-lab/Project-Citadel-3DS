...
#warning "PROJECT CITADEL NEW GAME HOME NORMALIZATION TEST 1 IS ACTIVE"
#endif
...
uchar create_initial_game_func(short undefined1, ulong undefined2, void *undefined3) {
    ...
    /*
     * PROJECT CITADEL NEW GAME HOME NORMALIZATION TEST 1
     *
     * A game created directly by New Game crashes when HOME suspends it,
     * while the exact same game becomes suspend-safe after passing through
     * load_game().  The first meaningful difference in load_game() is its
     * transient-state teardown:
     *
     *     empty_slate();
     *     closedown_game(TRUE);
     *
     * Preserve the setup-screen choices before that teardown, then create
     * the new game on the same clean foundation used by Load Game.
     */
    plr_obj = PLAYER_OBJ;
    for (i = 0; i < 4; i++)
    ...
    INFO("PROJECT CITADEL NEW GAME HOME T1: normalizing transient state");
    empty_slate();
    closedown_game(TRUE);
