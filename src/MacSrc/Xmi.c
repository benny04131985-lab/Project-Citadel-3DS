...
#warning "PROJECT CITADEL XMI V13A: native ADLMIDI sequencer is ACTIVE"
#include <stdarg.h>
#include <stdio.h>
...
#if defined(__3DS__) || defined(_3DS)
static void citadel_v13a_xmi_log(const char *fmt, ...)
{
    FILE *file;
    va_list args;

    file = fopen("MUSIC_3DS_V13A.log", "a");
    if (file == NULL)
        return;
...
#if defined(__3DS__) || defined(_3DS)
        citadel_v13a_xmi_log(
            "XMI OPEN FAIL path=%s",
            filename);
#endif
        return 0;
    }
...
#if defined(__3DS__) || defined(_3DS)
    citadel_v13a_xmi_log(
        "XMI READY path=%s tracks=%u mode=%s",
        filename,
        NumTracks,
        mode == Music_SoundBlaster ? "sblaster" : "genmidi");
#endif
...
#if defined(__3DS__) || defined(_3DS)
        citadel_v13a_xmi_log(
            "INIT FAIL reason=mutex error=%s",
            SDL_GetError());
#endif
        return;
    }
...
#if defined(__3DS__) || defined(_3DS)
        citadel_v13a_xmi_log(
            "INIT FAIL reason=thread error=%s",
            SDL_GetError());
#endif
        return;
    }
...
#if defined(__3DS__) || defined(_3DS)
    musicrate = citadel_3ds_audio_output_rate();
    musicdev = CreateMusicDevice(Music_AdlMidi);
    citadel_v13a_xmi_log(
        "INIT backend=ADLMIDI requested_rate=%d",
        musicrate);
#else
...
#if defined(__3DS__) || defined(_3DS)
    citadel_v13a_xmi_log(
        "INIT RESULT device=%p open=%d type=%d output=%u "
        "music_type=%s rate=%d",
        (void *)MusicDev,
        MusicDev != NULL ? MusicDev->isOpen : 0,
        MusicDev != NULL ? (int)MusicDev->deviceType : -1,
        MusicDev != NULL ? MusicDev->outputIndex : 0U,
