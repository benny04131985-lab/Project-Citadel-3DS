...
static bool citadel_gpu_initialize(void)
{
    ...
    v5_log("GPU V15F FROZEN FRAME ROUNDTRIP ready texture=%dx%d content=%dx%d "
...
static unsigned int citadel_gpu_refresh_palette565(void)
{
    ...
    if (citadel_gpu_palette_refreshes == 1 ||
        ...
        v5_log("GPU V15F PALETTE SYNC refresh=%lu mismatches=%u "
...
static bool citadel_gpu_present(SDL_Surface *surface, bool magenta)
{
    ...
    if (!magenta && !citadel_gpu_sync_order_logged) {
        v5_log("GPU V15F SYNC ORDER confirmed: "
    ...
    if (!magenta) {
        ...
        if (!citadel_gpu_cache_rebind_logged) {
            v5_log("GPU V15F CACHE REBIND active: "
...
static void v5_log(const char *fmt, ...)
{
    FILE *file = fopen("GPU_C2D_V15F.log", "a");
    va_list args;
...
void InitSDL() {
    ...
    remove("GPU_C2D_V15F.log");
    v5_log("PROJECT CITADEL V15F FROZEN FRAME ROUNDTRIP START | build=%s %s", __DATE__, __TIME__);
