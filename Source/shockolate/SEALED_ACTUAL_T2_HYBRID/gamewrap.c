...
#warning "PROJECT CITADEL NEW GAME HOME SELF-LOAD TEST 2 IS ACTIVE"
#endif
...
#if defined(__3DS__) || defined(_3DS)
    /*
     * PROJECT CITADEL NEW GAME HOME SELF-LOAD TEST 2
     *
     * Hardware testing proved:
     *   - a freshly created New Game crashes when HOME suspends it;
     *   - saving alone does not change that;
     *   - loading that exact save through load_game() makes HOME stable.
     *
     * Reproduce that known-good state transition automatically, after the
     * complete New Game has been constructed.  Use a temporary save file so
     * the user's visible save slots remain untouched.
     */
    {
        static const char *citadel_ngboot_file = "ngboot.dat";
        errtype citadel_save_result;
        errtype citadel_load_result = ERR_FOPEN;
        FILE *citadel_test_log =
            fopen("NEWGAME_HOME_T2.log", "wb");

        if (citadel_test_log != NULL) {
            fprintf(citadel_test_log,
                    "PROJECT CITADEL NEW GAME HOME T2 START "
                    "build=%s %s\n",
                    __DATE__,
                    __TIME__);
            fflush(citadel_test_log);
        }

        citadel_save_result =
            save_game((char *)citadel_ngboot_file,
                      "Project Citadel New Game normalization");

        if (citadel_test_log != NULL) {
            fprintf(citadel_test_log,
                    "TEMP SAVE result=%d file=%s\n",
                    (int)citadel_save_result,
                    citadel_ngboot_file);
            fflush(citadel_test_log);
        }

        if (citadel_save_result == OK) {
            citadel_load_result =
                load_game((char *)citadel_ngboot_file);
        }

        if (citadel_test_log != NULL) {
            fprintf(citadel_test_log,
                    "SELF LOAD result=%d\n",
                    (int)citadel_load_result);
            fflush(citadel_test_log);
        }

        if (citadel_save_result == OK &&
            citadel_load_result == OK) {
            int citadel_remove_result =
                remove(citadel_ngboot_file);

            if (citadel_test_log != NULL) {
                fprintf(citadel_test_log,
                        "TEMP DELETE result=%d\n",
                        citadel_remove_result);
                fflush(citadel_test_log);
            }
        }
        ...
        if (citadel_test_log != NULL) {
            fprintf(citadel_test_log,
                    "PROJECT CITADEL NEW GAME HOME T2 COMPLETE\n");
            fclose(citadel_test_log);
        }
    }
#endif
