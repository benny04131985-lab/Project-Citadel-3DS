...
static bool citadel_gpu_initialize(void)
{
    ...
    v5_log("GPU V15I SCREEN FORMAT before-targets "
                                          ...
                                          C3D_Tex *texture)
{
    ...
    if (texture == NULL) {
        v5_log("GPU V15I TEX %s NULL",
               label != NULL ? label : "(null)");
        return;
    }

    v5_log("GPU V15I TEX %s data=%p phys=0x%08lX "
           "width=%u height=%u fmt=%u size=%lu "
           "dim=0x%08lX param=0x%08lX lod=0x%08lX type=%u",
           label != NULL ? label : "(null)",
           texture->data,
...
static bool citadel_gpu_v15h_load_sheet(void)
{
    ...
    for (i = 0; i < sizeof(paths) / sizeof(paths[0]); ++i) {
        ...
        if (citadel_gpu_v15h_sheet != NULL) {
            v5_log("GPU V15I TEX3DS LOAD path=%s result=SUCCESS count=%lu",
                   paths[i],
        ...
        v5_log("GPU V15I TEX3DS LOAD path=%s result=FAIL",
               paths[i]);
    }
...
static bool citadel_gpu_v15h_initialize_control(void)
{
    ...
    if (C2D_SpriteSheetCount(citadel_gpu_v15h_sheet) < 1) {
        v5_log("GPU V15I TEX3DS LOAD FAIL reason=no-images");
        return false;
    }
    ...
    if (citadel_gpu_v15h_official_image.tex == NULL ||
        ...
        v5_log("GPU V15I TEX3DS LOAD FAIL reason=invalid-image");
        return false;
    }
    ...
    v5_log("GPU V15I SUBTEX official width=%u height=%u "
    ...
    if (citadel_gpu_v15h_official_image.tex->width !=
        ...
        v5_log("GPU V15I TEX3DS LOAD FAIL "
    ...
    if (!C3D_TexInit(&citadel_gpu_v15h_manual_texture,
                     ...
                     CITADEL_V15H_CONTROL_SIZE,
                     GPU_RGB565)) {
        v5_log("GPU V15I MANUAL INIT FAIL stage=C3D_TexInit");
        return false;
    }
    ...
    if (citadel_gpu_v15h_candidate == NULL) {
        v5_log("GPU V15I MANUAL INIT FAIL stage=linearAlloc bytes=%u",
    ...
    for (variant = 0; variant <= 5; ++variant) {
        ...
        v5_log("GPU V15I GOLDEN COMPARE variant=%d "
...
static bool citadel_gpu_upload_surface(SDL_Surface *surface)
{
    ...
    if (citadel_gpu_v15h_phase ==
        ...
        v5_log("GPU V15I PHASE begin=%s duration_ms=6000 "
    ...
    if (citadel_gpu_v15h_phase ==
        ...
        v5_log("GPU V15I PHASE complete; "
    ...
    if (!citadel_gpu_v15h_capture_done &&
        ...
        if (citadel_gpu_v15h_asset_ready) {
            ...
            citadel_gpu_v15h_phase =
                CITADEL_V15H_PHASE_COMPLETE;
            v5_log("GPU V15I PHASE SKIPPED reason=asset-unavailable");
        }
    }
...
static void v5_log(const char *fmt, ...)
{
    FILE *file = fopen("GPU_C2D_V15I.log", "a");
    va_list args;
...
void InitSDL() {
    ...
    remove("GPU_C2D_V15I.log");
    v5_log("PROJECT CITADEL V15I SDL RGBA8 TARGET OUTPUT START | build=%s %s", __DATE__, __TIME__);
