...
#define CITADEL_V21_PRIMER_GAME_FRAMES 8U

static unsigned int citadel_3ds_v21_setup_frames = 0;
static bool citadel_3ds_v21_primer_done = false;
static unsigned int citadel_3ds_v21_game_frames = 0;
static bool citadel_3ds_v21_game_primer_done = false;
...
static void citadel_3ds_v21_run_minimal_primer(void)
{
    ...
    loopmark("V21A PRIMER BEGIN: waiting for completed frame VBlank");
    gspWaitForVBlank();
...
void mainloop(int argc, char *argv[]) {
    ...
    while (_current_loop >= 0 && gPlayingGame) {
         ...
         */
        /*
         * V21E: T7 remains compiled for rollback comparison, but its automatic
         * Save/Load callback is intentionally dormant. New Game proceeds normally.
         */
        (void)loop_at_frame_start;
#endif
        ...
        /*
         * V21E: New Game changes from Journey's SETUP_LOOP presentation to the
         * live GAME_LOOP presentation. Re-run the exact proven GSP sequence once
         * the real gameplay framebuffer has completed eight stable frames.
         *
         * This replaces T7's visible Save/Load lifecycle with an invisible
         * framebuffer-local primer. No save, load, wrapper, input, or game-state
         * callback is invoked.
         */
        if (!citadel_3ds_v21_game_primer_done &&
            loop_at_frame_start == GAME_LOOP &&
            _current_loop == GAME_LOOP &&
            gPlayingGame) {
            citadel_3ds_v21_game_frames++;
            if (citadel_3ds_v21_game_frames >= CITADEL_V21_PRIMER_GAME_FRAMES) {
                citadel_3ds_v21_game_primer_done = true;
                loopmark("V21E LIVE GAME PRIMER BEGIN");
                citadel_3ds_v21_run_minimal_primer();
                loopmark("V21E LIVE GAME PRIMER RETURNED");
            }
        }
#endif
