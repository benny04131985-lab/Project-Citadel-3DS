...
#warning "PROJECT CITADEL C3D R3D6B2 NATIVE FOREGROUND DIRECT1: full native-stereo right-frame compositor is bypassed and fused at final foreground samples"
#warning "PROJECT CITADEL C3D R3D6B3 RIGHT BUFFER DIRECT1: right software residual writes directly into retained eye buffer; full-frame right memcpy capture is bypassed"
/* -------------------------------------------------------------------------
 ...
 *
 * Native stereo no longer needs a complete software-composed right frame.
 * Keep the right pre-overlay residual for eye-specific unsupported software
 * objects, capture only the station-game rows from the left eye as an overlay
 * baseline, then fuse shared flat overlays at final 400x240 foreground samples.
 *
 * Exceptional/non-native modes retain the proven full S2 compositor.
 */
static bool citadel_3ds_stereo_direct_foreground_candidate = false;
static bool citadel_3ds_stereo_direct_foreground_ready = false;

unsigned long long citadel_r3d6b2_direct_foreground_frames = 0;
unsigned long long citadel_r3d6b2_right_residual_captures = 0;
unsigned long long citadel_r3d6b2_left_foreground_row_captures = 0;
unsigned long long citadel_r3d6b2_left_full_capture_bypassed = 0;
unsigned long long citadel_r3d6b2_compose_scans_bypassed = 0;
unsigned long long citadel_r3d6b2_legacy_compositor_frames = 0;
unsigned long long citadel_r3d6b2_direct_fallbacks = 0;
unsigned long long citadel_r3d6b2_foreground_capture_bytes = 0;

/* PROJECT CITADEL C3D R3D6B3 RIGHT BUFFER DIRECT1
 *
 * The right software pass can use the retained right-world buffer itself as
 * grd_screen_canvas backing. The existing copy-to-screen step therefore lands
 * directly in the retained residual and no second full-frame memcpy is needed.
 */
static uchar *citadel_r3d6b3_live_screen_bits = NULL;
static bool citadel_r3d6b3_right_buffer_redirected = false;

unsigned long long citadel_r3d6b3_direct_right_buffer_frames = 0;
unsigned long long citadel_r3d6b3_right_full_capture_bypassed = 0;
unsigned long long citadel_r3d6b3_right_memcpy_fallback_captures = 0;
unsigned long long citadel_r3d6b3_right_memcpy_bytes_saved = 0;
unsigned long long citadel_r3d6b3_screen_restore_failures = 0;

/* PROJECT CITADEL C3D R3D7A AVP STEREO PARITY1
 *
 * A1 deliberately schedules exactly one Shock/fr_rend scene traversal for
 * stereo. The captured LEFT native stream and completed flat foreground are
 * presented to both physical top-eye targets at zero disparity. No right
 * software residual, baseline capture, or right-frame compositor is used.
 */
unsigned long long citadel_r3d7a_left_parity_passes = 0;
unsigned long long citadel_r3d7a_left_capture_bypassed = 0;
unsigned long long citadel_r3d7a_legacy_compose_bypassed = 0;
unsigned long long citadel_r3d7a_unexpected_right_passes = 0;
...
static void citadel_r3d6b3_restore_live_screen_buffer(void)
{
    if (!citadel_r3d6b3_right_buffer_redirected)
        return;

    if (grd_screen_canvas != NULL &&
        citadel_r3d6b3_live_screen_bits != NULL) {
        grd_screen_canvas->bm.bits = citadel_r3d6b3_live_screen_bits;
    } else {
        ++citadel_r3d6b3_screen_restore_failures;
    }

    citadel_r3d6b3_right_buffer_redirected = false;
}

static bool citadel_r3d6b3_redirect_right_buffer(void)
{
    if (grd_screen_canvas == NULL ||
        grd_screen_canvas->bm.bits == NULL ||
        citadel_3ds_stereo_right_world == NULL ||
        citadel_3ds_stereo_buffer_bytes == 0)
        return false;

    /*
     * Save the real screen backing immediately before redirecting. This makes
     * the helper robust even if begin_frame was entered after a non-stereo
     * frame or a wrapper transition.
     */
    citadel_r3d6b3_live_screen_bits = grd_screen_canvas->bm.bits;
    grd_screen_canvas->bm.bits = citadel_3ds_stereo_right_world;
    citadel_r3d6b3_right_buffer_redirected = true;
    return true;
}
...
{
    /*
     * B3 fail-safe: a prior frame must never leave the public screen canvas
     * redirected, even if an exceptional path exited between eye passes.
     */
    citadel_r3d6b3_restore_live_screen_buffer();
    ...
    citadel_3ds_stereo_direct_foreground_candidate = false;
    citadel_3ds_stereo_direct_foreground_ready = false;
...
void citadel_3ds_stereo_begin_right_pass(void)
{
    ...
    if (citadel_3ds_stereo_pass == CITADEL_3DS_STEREO_PASS_RIGHT &&
        citadel_3ds_stereo_prepare_buffers()) {
        (void)citadel_r3d6b3_redirect_right_buffer();
    }
}
...
{
    /*
     * The left pass must always render to the real screen backing. This is
     * also the normal restoration point for the B3 right-buffer redirect.
     */
    citadel_r3d6b3_restore_live_screen_buffer();
...
/*
 * B2 native-direct overlay baseline.
 *
 * The displayed top station view occupies 328/480 of the reference software
 * surface. Only those rows can be sampled by the 400x240 foreground texture,
 * so copying the lower 152 rows is pure legacy compositor work.
 */
static bool citadel_3ds_stereo_capture_foreground_rows(uchar *destination)
{
    ...
    screen = &grd_screen_canvas->bm;
    rows = (screen->h * 328 + 479) / 480;
    if (rows < 1)
        rows = 1;
    if (rows > screen->h)
        rows = screen->h;

    bytes = (size_t)screen->row * (size_t)rows;
    memcpy(destination, screen->bits, bytes);
    citadel_r3d6b2_foreground_capture_bytes +=
...
/*
 * B2 emergency bridge back to S2 composition. This is called only if the
 * direct 400x240 foreground fusion cannot be consumed safely by presentation.
 * The top station rows of left_world are the exact pre-overlay baseline, which
 * is all the split-layout top presentation can sample.
 */
int citadel_3ds_stereo_force_legacy_compositor(void)
{
    if (!citadel_3ds_stereo_direct_foreground_ready)
        return citadel_3ds_stereo_right_frame_valid ? 1 : 0;

    citadel_3ds_stereo_direct_foreground_ready = false;
    citadel_3ds_stereo_compose_right_frame();

    if (!citadel_3ds_stereo_right_frame_valid)
        return 0;

    ++citadel_r3d6b2_direct_fallbacks;
    ++citadel_r3d6b2_legacy_compositor_frames;
    return 1;
}
                                                 ...
                                                 int *pitch, float *slider)
{
    ...
    if (citadel_3ds_stereo_direct_foreground_ready) {
        if (!citadel_3ds_stereo_right_world_valid)
            return NULL;
        pixels = citadel_3ds_stereo_right_world;
    } else {
        if (!citadel_3ds_stereo_right_frame_valid)
            return NULL;
        pixels = citadel_3ds_stereo_right_frame;
    }
...
int citadel_3ds_stereo_direct_foreground_active(void)
{
    return citadel_3ds_stereo_direct_foreground_ready ? 1 : 0;
}

const uchar *citadel_3ds_stereo_get_left_world_baseline(
    int *width, int *height, int *pitch)
{
    ...
    if (height != NULL) *height = citadel_3ds_stereo_height;
    if (pitch != NULL) *pitch = citadel_3ds_stereo_pitch;
    return citadel_3ds_stereo_left_world;
}

int citadel_3ds_stereo_get_convergence_shift(void)
{
    int shift =
        (int)(citadel_3ds_stereo_slider_curve *
              (float)CITADEL_3DS_STEREO_MAX_CONVERGENCE_PIXELS + 0.5f);
    ...
    if (shift < 0)
        shift = 0;
    if (shift > CITADEL_3DS_STEREO_MAX_CONVERGENCE_PIXELS)
        shift = CITADEL_3DS_STEREO_MAX_CONVERGENCE_PIXELS;
    return shift;
}
#endif
...
#if defined(__3DS__) || defined(_3DS)
    if (((_fr_curflags & (FR_PICKUPM_MASK | FR_HACKCAM_MASK)) == 0) &&
        ((_fr->flags & FR_CURVIEW_MASK) == FR_CURVIEW_STRT)) {
...
#if defined(__3DS__) || defined(_3DS)
    /*
     * R3D4A: in active mono takeover, the software canvas is a foreground
     * ownership layer. Non-terrain draws retain their real palette colors;
     * captured terrain polygons later erase their coverage back to key zero
     * in original painter order. Pause, wrappers, stereo and Legacy retain
     * the complete ordinary software frame.
     */
    if (citadel_native_world_foreground_render_active())
        gr_clear(0);
    else
...
int fr_send_view(void) {
    ...
    if (citadel_3ds_stereo_right_pass_active()) {
        /*
         * R3D7A schedules no physical right engine pass. Keep the proven B3
         * implementation compiled as a safety tripwire/reference only.
         */
        ++citadel_r3d7a_unexpected_right_passes;
        citadel_3ds_stereo_copy_world_to_screen(ok_to_double);

        if (citadel_r3d6b3_right_buffer_redirected &&
            grd_screen_canvas != NULL &&
            grd_screen_canvas->bm.bits == citadel_3ds_stereo_right_world) {
            /*
             * B3: Fast_FullScreen_Copy/Fast_Slot_Copy just wrote the residual
             * directly into its retained destination. No capture memcpy.
             */
            citadel_3ds_stereo_right_world_valid = true;
            ++citadel_r3d6b3_direct_right_buffer_frames;
            ++citadel_r3d6b3_right_full_capture_bypassed;
            citadel_r3d6b3_right_memcpy_bytes_saved +=
                (unsigned long long)citadel_3ds_stereo_buffer_bytes;
        } else {
            /*
             * Fail closed to the exact B2 behavior if redirection was not
             * available for any reason.
             */
            citadel_3ds_stereo_right_world_valid =
                citadel_3ds_stereo_capture_screen(
                    citadel_3ds_stereo_right_world);

            if (citadel_3ds_stereo_right_world_valid) {
                ++citadel_r3d6b2_right_residual_captures;
                ++citadel_r3d6b3_right_memcpy_fallback_captures;
            }
        }

        /*
         * Restore the real public screen buffer before any later game/UI work.
         */
        citadel_r3d6b3_restore_live_screen_buffer();

        /*
         * Native foreground rendering means the world itself is already owned
         * by Citro3D. Preserve the right software residual for unsupported
         * objects, but do not commit to the direct path until the matching
         * left pre-overlay baseline has also been captured successfully.
         */
        citadel_3ds_stereo_direct_foreground_candidate =
            citadel_3ds_stereo_right_world_valid &&
            citadel_native_world_foreground_render_active();
    ...
    if (citadel_3ds_stereo_left_pass_active()) {
         ...
         * PROJECT CITADEL C3D R3D7A AVP STEREO PARITY1
         *
         * This is the one and only stereo engine pass. Copy its software world
         * to the real public screen so the normal single overlay callback can
         * complete the flat HUD/surround exactly once.
         *
         * Deliberately invalidate every old right-residual/baseline state.
         * The presenter will therefore reuse this same LEFT native stream and
         * this same completed flat foreground for the physical RIGHT target.
         */
        citadel_3ds_stereo_copy_world_to_screen(ok_to_double);
    ...
    if (citadel_3ds_stereo_left_pass_active()) {
        /*
         * R3D7A: there is intentionally no software RIGHT frame to compose.
         * The completed LEFT surface is the single flat foreground source for
         * both physical eyes. Keep every legacy stereo-frame flag invalid.
         */
        citadel_3ds_stereo_direct_foreground_ready = false;
        citadel_3ds_stereo_direct_foreground_candidate = false;
        citadel_3ds_stereo_right_world_valid = false;
