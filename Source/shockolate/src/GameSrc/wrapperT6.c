...
#warning "PROJECT CITADEL NEW GAME HOME NORMALIZER T6 is ACTIVE"

/*
 * T6 reproduces the one lifecycle proven HOME-safe on hardware:
 *
 *   fresh New Game
 *     -> complete a real GAME_LOOP frame
 *     -> ordinary save
 *     -> complete another real GAME_LOOP frame
 *     -> open the ordinary in-game load wrapper context
 *     -> ordinary load_game()
 *     -> ordinary wrapper_panel_close()
 *
 * It does not call object_data_load(), player_create_initial(),
 * clear_player_data, change_mode_func(), or the opening Continue helper.
 * No numbered user save slot is touched.
 */
typedef enum Citadel3dsT6Stage {
    CITADEL_T6_IDLE = 0,
    CITADEL_T6_SAVE_AFTER_GAME_FRAME,
    CITADEL_T6_LOAD_AFTER_NEXT_GAME_FRAME,
    CITADEL_T6_COMPLETE,
    CITADEL_T6_FAILED
} Citadel3dsT6Stage;

static Citadel3dsT6Stage citadel_3ds_t6_stage = CITADEL_T6_IDLE;
static char citadel_3ds_t6_temp_file[] = "NewGameT6.dat";
static char citadel_3ds_t6_temp_comment[] = "3DS New Game Normalizer";

static void citadel_3ds_t6_log(const char *message)
{
    FILE *file = fopen("NEWGAME_HOME_T6.log", "a");
...
static void citadel_3ds_t6_log_result(const char *operation,
                                      errtype result)
{
    FILE *file = fopen("NEWGAME_HOME_T6.log", "a");
    ...
    fprintf(file, "%s result=%d\n", operation, (int)result);
    fflush(file);
    fclose(file);
}

void citadel_3ds_arm_newgame_home_t6(void)
{
    remove("NEWGAME_HOME_T6.log");
    remove(citadel_3ds_t6_temp_file);

    citadel_3ds_t6_stage = CITADEL_T6_SAVE_AFTER_GAME_FRAME;

    citadel_3ds_t6_log(
        "T6 ARMED: finalized New Game; waiting for first complete GAME_LOOP frame");
    citadel_3ds_t6_log("T6 BUILD: " __DATE__ " " __TIME__);
}

void citadel_3ds_newgame_home_t6_tick(void)
{
    errtype result;
    int delete_result;

    if (citadel_3ds_t6_stage == CITADEL_T6_IDLE ||
        citadel_3ds_t6_stage == CITADEL_T6_COMPLETE ||
        citadel_3ds_t6_stage == CITADEL_T6_FAILED)
        return;

    if (citadel_3ds_t6_stage == CITADEL_T6_SAVE_AFTER_GAME_FRAME) {
        citadel_3ds_t6_log(
            "T6 FRAME 1 COMPLETE: beginning ordinary temporary save");

        begin_wait();
        result = save_game(citadel_3ds_t6_temp_file,
                           citadel_3ds_t6_temp_comment);
        end_wait();

        citadel_3ds_t6_log_result("T6 TEMP SAVE", result);

        if (result != OK) {
            citadel_3ds_t6_stage = CITADEL_T6_FAILED;
            citadel_3ds_t6_log(
                "T6 FAILED: temporary save failed; no load attempted");
            return;
        }

        citadel_3ds_t6_stage = CITADEL_T6_LOAD_AFTER_NEXT_GAME_FRAME;
        citadel_3ds_t6_log(
            "T6 SAVE COMPLETE: returning to GAME_LOOP for one full frame");
        return;
    }

    if (citadel_3ds_t6_stage == CITADEL_T6_LOAD_AFTER_NEXT_GAME_FRAME) {
        if (wrapper_panel_on) {
            citadel_3ds_t6_log(
                "T6 LOAD DELAYED: wrapper already open; waiting one more frame");
            return;
        }

        citadel_3ds_t6_log(
            "T6 FRAME 2 COMPLETE: opening ordinary in-game load wrapper context");

        /*
         * Recreate the same paused/UI-owned context used by Options -> Load.
         * The panel is opened and closed synchronously, so no user interaction
         * or numbered save slot is involved.
         */
        wrapper_start(load_screen_init);

        begin_wait();
        result = load_game(citadel_3ds_t6_temp_file);
        end_wait();

        citadel_3ds_t6_log_result("T6 TEMP LOAD", result);

        /* Match load_dealfunc(): always leave through normal wrapper cleanup. */
        wrapper_panel_close(TRUE);
        citadel_3ds_t6_log("T6 WRAPPER CLOSE COMPLETE");

        delete_result = remove(citadel_3ds_t6_temp_file);
        {
            FILE *file = fopen("NEWGAME_HOME_T6.log", "a");
            if (file != NULL) {
                fprintf(file,
                        "T6 TEMP DELETE result=%d file=%s\n",
                        delete_result,
                        citadel_3ds_t6_temp_file);
                fflush(file);
                fclose(file);
            }
        }
        ...
        if (result != OK) {
            citadel_3ds_t6_stage = CITADEL_T6_FAILED;
            citadel_3ds_t6_log("T6 FAILED: temporary load failed");
            return;
        }
        ...
        citadel_3ds_t6_stage = CITADEL_T6_COMPLETE;
        citadel_3ds_t6_log(
            "T6 COMPLETE: New Game rebuilt through proven in-game load lifecycle");
    }
}
