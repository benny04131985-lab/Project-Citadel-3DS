...
/*
 * --------------------------------------------------------------------------
 * TEMPORARY 3DS CUTSCENE BYPASS
 * --------------------------------------------------------------------------
 * The #warning line should appear in the compiler output. If it does not,
 * this is not the cutsloop.c used by the active build.
 */
#warning "PROJECT CITADEL V4: cutsloop directly calls setup_start"

static void cutscene_bypass_mark(int id, bool show_credits)
{
    FILE *file = fopen("cutscene_bypass.log", "a");

    if (file == NULL)
        return;

    fprintf(file,
            "play_cutscene bypassed: id=%d show_credits=%d build=%s %s\n",
            id,
            show_credits ? 1 : 0,
            __DATE__,
            __TIME__);
...
static void direct_setup_mark(const char *message)
{
    FILE *file = fopen("DIRECT_SETUP_V4.log", "a");

    if (file == NULL)
        return;

    fprintf(file,
            "V4 %s | build=%s %s current=%d new=%d change=0x%08X\n",
            message,
            __DATE__,
            __TIME__,
            (int)_current_loop,
...
{
    /*
     * This function is already proven to execute on the 3DS. Use it as the
     * reliable place to initialize the setup/menu state directly.
     */
    remove("DIRECT_SETUP_V4.log");
    cutscene_bypass_mark(id, show_credits);
    direct_setup_mark("ENTERED play_cutscene bypass");

    _current_loop = SETUP_LOOP;
    _new_mode = SETUP_LOOP;
    chg_unset_flg(GL_CHG_LOOP);

    is_first_frame = FALSE;
    done_playing_movie = TRUE;

    direct_setup_mark("BEFORE direct setup_start()");
    setup_start();
    direct_setup_mark("AFTER direct setup_start()");

    /*
     * Defensive cleanup in case setup_start() itself schedules the intro.
     */
    _current_loop = SETUP_LOOP;
    _new_mode = SETUP_LOOP;
    chg_unset_flg(GL_CHG_LOOP);
    direct_setup_mark("RETURNING with SETUP_LOOP forced");

    return 0;

#if 0
    /*
     * Original implementation preserved for later restoration.
     * Remove the bypass above and change #if 0 to #if 1 when cutscenes are
