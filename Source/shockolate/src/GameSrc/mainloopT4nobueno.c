...
void mainloop(int argc, char *argv[]) {
    ...
    while (_current_loop >= 0 && gPlayingGame) {
         ...
         *
         * SDL's N3DS event backend already owns aptMainLoop(), so do not call
         * aptMainLoop() a second time. Instead, make SDL/APT the first active
         * operation of every Shock frame:
         *
         *   1. wait for the previously submitted Citro3D frame to finish;
         *   2. pump SDL events so HOME/sleep is serviced before game work;
         *   3. freeze the whole frame if APT still reports suspension.
         */
        if (log_this_frame)
         ...
         * Stop before timers, input, simulation, cursor work, or rendering.
         */
        ...
        /*
         * aptMainLoop normally blocks until restore. If this SDL/libctru
         * combination returns while APT is still suspended, run absolutely no
         * Shock or GPU work; idle briefly and service SDL/APT again.
         */
