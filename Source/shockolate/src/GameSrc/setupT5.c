...
#warning "PROJECT CITADEL KEYBOARD V3: visible-top native keyboard is ACTIVE"
#warning "PROJECT CITADEL NEW GAME EXACT LOAD T5 IS ACTIVE"
#endif
...
/*
 * PROJECT CITADEL NEW GAME EXACT LOAD T5
 *
 * New Game is first created and finalized normally. At the end of the
 * setup callback it is saved to a hidden file. The following setup-loop
 * dispatch then enters the exact same initialization/load routine used by
 * the Continue screen.
 */
static bool citadel_newgame_exact_load_pending = false;
static char citadel_newgame_boot_filename[] = "NewGameBoot.dat";
static char citadel_newgame_boot_comment[] = "New Game Boot";

static void citadel_t5_log(const char *format, ...)
{
    FILE *file;
    va_list arguments;

    file = fopen("NEWGAME_EXACT_LOAD_T5.log", "a");
    if (file == NULL)
        return;
...
static void citadel_3ds_set_default_player_name(void)
{
    if (player_struct.name[0] == '\0') {
        strncpy(player_struct.name, "Player", sizeof(player_struct.name) - 1);
        player_struct.name[sizeof(player_struct.name) - 1] = '\0';
    }
}

static void citadel_3ds_open_name_keyboard(void)
{
    SwkbdState keyboard;
    SwkbdButton button;
    char entered_name[sizeof(player_struct.name)];
    size_t source_index;
    size_t dest_index = 0;

    citadel_3ds_set_default_player_name();

    strncpy(entered_name, player_struct.name, sizeof(entered_name) - 1);
    entered_name[sizeof(entered_name) - 1] = '\0';

    /*
     * Western keeps the result compatible with Shock's original single-byte
     * name field. Two buttons give us Cancel and OK.
     */
    swkbdInit(&keyboard,
              SWKBD_TYPE_WESTERN,
              2,
              (int)sizeof(entered_name) - 1);

    swkbdSetHintText(&keyboard, "Enter hacker name");
    swkbdSetInitialText(&keyboard, entered_name);
    swkbdSetValidation(&keyboard, SWKBD_NOTEMPTY_NOTBLANK, 0, 0);
    /*
     * Keep Shock's name-entry screen visible above the lower-screen keyboard.
     */
    swkbdSetFeatures(&keyboard, SWKBD_DEFAULT_QWERTY);
    swkbdSetButton(&keyboard, SWKBD_BUTTON_LEFT, "Cancel", false);
    swkbdSetButton(&keyboard, SWKBD_BUTTON_RIGHT, "OK", true);

    button = swkbdInputText(&keyboard, entered_name, sizeof(entered_name));

    if (button == SWKBD_BUTTON_CONFIRM) {
        /*
         * Shock's original font/input path accepts the characters in
         * valid_char_string. Filter unsupported UTF-8/punctuation instead of
         * feeding bytes that the 1994 text renderer cannot display.
         */
        for (source_index = 0;
             entered_name[source_index] != '\0' &&
             dest_index < sizeof(player_struct.name) - 1;
             ++source_index) {
            unsigned char character = (unsigned char)entered_name[source_index];

            if (character < 128 &&
                strchr(valid_char_string, (int)character) != NULL) {
                player_struct.name[dest_index++] = (char)character;
            }
        }

        player_struct.name[dest_index] = '\0';
        citadel_3ds_set_default_player_name();
    }

    /*
     * Redraw the complete difficulty page after the system applet restores
     * control, then discard any stale click/key events used to close it.
     */
...
/*
 * This is the literal Continue/Load initialization sequence, centralized so
 * the ordinary Continue screen and the New Game normalization pass execute
 * the same code.
 *
 * which_slot >= 0:
 *     obtain the filename with Poke_SaveName(), exactly where the original
 *     Continue path did it.
 *
 * which_slot < 0:
 *     use direct_filename for the hidden finalized-New-Game save.
 */
static errtype citadel_3ds_exact_load_path(int which_slot,
                                           char *direct_filename)
{
    errtype retval;
    char *filename = direct_filename;
...
#if defined(__3DS__) || defined(_3DS)
        /*
         * T5 deliberately routes ordinary Continue through the shared exact
         * loader too. This guarantees New Game and Continue cannot silently
         * diverge inside setup.c.
         */
        retval = citadel_3ds_exact_load_path(which_slot, NULL);
#else
...
#if defined(__3DS__) || defined(_3DS)
    remove("NEWGAME_EXACT_LOAD_T5.log");
    citadel_newgame_exact_load_pending = false;
    citadel_t5_log("T5 NEW GAME FINALIZATION BEGIN build=%s %s",
                   __DATE__,
                   __TIME__);
#endif
...
#if defined(__3DS__) || defined(_3DS)
    /*
     * Every New Game quest variable, difficulty value, player version,
     * status-bio transition, and rendering override is finalized above.
     * Save only now, then let go_and_start_the_game_already() return fully.
     */
    {
        errtype save_result =
            save_game(citadel_newgame_boot_filename,
                      citadel_newgame_boot_comment);

        citadel_t5_log("T5 FINAL TEMP SAVE result=%d file=%s",
                       (int)save_result,
                       citadel_newgame_boot_filename);

        if (save_result == OK) {
            citadel_newgame_exact_load_pending = true;
            citadel_t5_log("T5 EXACT LOAD QUEUED FOR SETUP LOOP");
        } else {
            citadel_t5_log("T5 TEMP SAVE FAILED; ORIGINAL NEW GAME CONTINUES");
        }
    }
#endif
}
...
#if defined(__3DS__) || defined(_3DS)
            /*
             * Include both the NAME label and the editable value in the touch
             * target. ev->pos is already in Shock's setup-screen coordinates.
             */
            if ((ev->pos.x >= DIFF_NAME_X) &&
                (ev->pos.x <= DIFF_NAME_X2) &&
                (ev->pos.y >= DIFF_NAME_Y) &&
                (ev->pos.y <= DIFF_NAME_Y2)) {
                citadel_3ds_open_name_keyboard();
                break;
            }
#endif
...
#if defined(__3DS__) || defined(_3DS)
    /*
     * This runs only after the New Game input callback and
     * go_and_start_the_game_already() have returned. The load below is not
     * re-entrant inside create_initial_game_func() or New Game finalization.
     */
    if (citadel_newgame_exact_load_pending) {
        errtype load_result;

        citadel_newgame_exact_load_pending = false;
        citadel_t5_log("T5 EXACT LOAD ENTRY FROM SETUP LOOP");

        load_result =
            citadel_3ds_exact_load_path(
                -1,
                citadel_newgame_boot_filename);

        citadel_t5_log("T5 EXACT LOAD result=%d",
                       (int)load_result);

        if (load_result == OK) {
            int remove_result =
                remove(citadel_newgame_boot_filename);

            citadel_t5_log("T5 TEMP DELETE result=%d",
                           remove_result);
            citadel_t5_log("T5 COMPLETE: NEW GAME NOW USES CONTINUE LOAD PATH");
        } else {
            citadel_t5_log("T5 FAILED: TEMP FILE LEFT FOR INSPECTION");
        }
...
#if defined(__3DS__) || defined(_3DS)
    /*
     * setup_init() may already have loaded a configured/default name. Preserve
     * it; the old unconditional clear is what left the 3DS screen blank.
     */
    citadel_3ds_set_default_player_name();
#else
