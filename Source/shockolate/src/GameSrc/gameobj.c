...
void _fr_draw_tmtile(grs_bitmap *draw_bm, int col_val, g3s_phandle *plst, uchar dblface, uchar use_lighting) {
    int t_off_l, t_off_r;
#if defined(__3DS__) || defined(_3DS)
    const int citadel_native_door_scope =
        dblface &&
        draw_bm != NULL &&
        _fr_cobj != NULL &&
        _fr_cobj->obclass == CLASS_DOOR;

    if (citadel_native_door_scope)
        citadel_native_world_set_surface_kind(CITADEL_NATIVE_SURFACE_DOOR);
#endif
...
#if defined(__3DS__) || defined(_3DS)
    if (citadel_native_door_scope)
        citadel_native_world_set_surface_kind(CITADEL_NATIVE_SURFACE_NONE);
#endif
}
...
// when I have a more coherent plan WRT it.
...
#if defined(__3DS__) || defined(_3DS)
    {
        const int citadel_native_bitmap_scope =
            citadel_native_world_foreground_render_active() &&
            ((_fr_curflags & (FR_PICKUPM_MASK | FR_HACKCAM_MASK)) == 0) &&
            !IS_HUDOBJ(cobjid);

        if (citadel_native_bitmap_scope)
            citadel_native_world_begin_bitmap_object();
#endif
...
#if defined(__3DS__) || defined(_3DS)
        if (citadel_native_bitmap_scope)
            citadel_native_world_end_bitmap_object();
    }
#endif
        if (global_fullmap->cyber)
            if (obj_ICE_ICE_BABY(_fr_cobj))
    ...
    case FAUBJ_CRIT: {
        int view;
#if defined(__3DS__) || defined(_3DS)
        const int citadel_native_critter_bitmap_scope =
            obj_type == FAUBJ_CRIT &&
            citadel_native_world_foreground_render_active() &&
            ((_fr_curflags &
              (FR_PICKUPM_MASK | FR_HACKCAM_MASK)) == 0) &&
            !IS_HUDOBJ(cobjid);
        const int citadel_native_multiview_bitmap_scope =
            obj_type == FAUBJ_MULTIVIEW &&
            citadel_native_world_foreground_render_active() &&
            ((_fr_curflags &
              (FR_PICKUPM_MASK | FR_HACKCAM_MASK)) == 0) &&
            !IS_HUDOBJ(cobjid);
...
#if defined(__3DS__) || defined(_3DS)
                    if (citadel_native_critter_bitmap_scope)
                        citadel_native_world_note_critter_special_software();
#endif
...
#if defined(__3DS__) || defined(_3DS)
            if (citadel_native_multiview_bitmap_scope) {
                citadel_native_world_begin_bitmap_object_category(
...
#if defined(__3DS__) || defined(_3DS)
        {
            const int citadel_native_texbitmap_scope =
                obj_type == FAUBJ_TEXBITMAP &&
                (_fr_cobj->obclass == CLASS_BIGSTUFF ||
                 _fr_cobj->obclass == CLASS_SMALLSTUFF ||
                 _fr_cobj->obclass == CLASS_FIXTURE);

            if (citadel_native_texbitmap_scope)
