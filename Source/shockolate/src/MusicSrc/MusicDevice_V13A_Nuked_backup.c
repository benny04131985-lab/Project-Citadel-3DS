...
#warning "PROJECT CITADEL MUSICDEVICE V13A: native OPL synthesis is ACTIVE"
#endif
...
static int AdlMidiInit(MusicDevice *dev, const unsigned int outputIndex, unsigned samplerate)
{
    ...
    /* Keep Shockolate's already-proven ADLMIDI emulator for V13A. */
...
static void AdlMidiGenerate(MusicDevice *dev, short *samples, int numframes)
{
    ...
    /* Preserve the historical 2x gain without signed-short wraparound. */
    short *sample = samples;
    for (int i = 0; i < numSamples; ++i, ++sample)
    {
        int value = (int)(*sample) * 2;
