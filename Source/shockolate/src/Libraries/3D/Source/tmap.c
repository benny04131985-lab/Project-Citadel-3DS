...
static void citadel_native_draw_software_tmap(
    grs_bitmap *bitmap,
    int vertex_count,
    grs_vertex **vertices,
    grs_tmap_info *info,
    int native_capture_action)
{
    void (*mapper)(grs_bitmap *, int, grs_vertex **, grs_tmap_info *) =
        (void (*)(grs_bitmap *, int, grs_vertex **, grs_tmap_info *))
        tmap_func;

    if (native_capture_action == CITADEL_NATIVE_CAPTURE_TERRAIN_ERASE) {
        /*
         * R3D4A: preserve software painter-order visibility without running
         * the textured CPU mapper. A captured opaque terrain polygon erases
         * any earlier foreground pixels to the alpha key. Objects/effects
         * drawn later remain visible, exactly following original draw order.
         */
        gr_upoly(0, vertex_count, vertices);
        return;
    }

    if (native_capture_action == CITADEL_NATIVE_CAPTURE_DOOR_ERASE) {
        /*
         * R3D5A2: native doors still perform the cheap key-zero polygon erase
         * at their original painter-order position. This removes earlier
         * decals and objects behind the opaque door without running the
         * textured software mapper.
         */
        gr_upoly(0, vertex_count, vertices);
        return;
    }

    if (native_capture_action == CITADEL_NATIVE_CAPTURE_DOOR_ALPHA_ERASE) {
        /*
         * R3D5A3: native binary-alpha doors require an alpha-shaped erase in
         * the software foreground. The helper reuses the proven perspective
         * mapper on a 0/1 mask, then clears only covered door texels to key 0.
         * If mask generation fails, keep the original software door draw as a
         * visually safe fallback; the native copy remains hidden underneath.
         */
        if (citadel_native_world_apply_transparent_door_erase(
                vertex_count,
                vertices,
                info)) {
            return;
        ...
        mapper(bitmap, vertex_count, vertices, info);
        return;
    }

    if (native_capture_action == CITADEL_NATIVE_CAPTURE_TEXBITMAP_ERASE) {
        /*
         * R3D5B2D: opaque non-door TEXBITMAP objects use the same cheap
         * painter-order key erase as native opaque doors.
         */
        gr_upoly(0, vertex_count, vertices);
        return;
    }

    if (native_capture_action ==
        CITADEL_NATIVE_CAPTURE_TEXBITMAP_ALPHA_ERASE) {
        /*
         * R3D5B2D: binary-alpha BIGSTUFF/SMALLSTUFF/FIXTURE TEXBITMAP
         * objects keep exact software occlusion by erasing only nonzero
         * source texels. Failure falls back to the original mapper.
         */
        if (citadel_native_world_apply_transparent_texbitmap_erase(
                vertex_count,
                vertices,
                info)) {
            return;
    ...
    if (native_capture_action == CITADEL_NATIVE_CAPTURE_SUPPRESS) {
        return;
    }

    /* Unsupported/transparent categories remain on the software renderer. */
    mapper(bitmap, vertex_count, vertices, info);
}
#endif
...
int draw_tmap_common(int n, g3s_phandle *vp, grs_bitmap *bm) {
    ...
    int i, temp;
#if defined(__3DS__) || defined(_3DS)
    int native_capture_action = CITADEL_NATIVE_CAPTURE_SOFTWARE;
#endif
    grs_vertex *cur_vert;
...
#if defined(__3DS__) || defined(_3DS)
    native_capture_action =
        citadel_native_world_capture_tmap(
            _n_verts, _vbuf2, bm, light_flag);
#endif

    if (!light_flag) {
#if defined(__3DS__) || defined(_3DS)
        citadel_native_draw_software_tmap(
            bm, _n_verts, p_vpl, &ti,
            native_capture_action);
#else
