...
grs_vertex **g3_bitmap_common(grs_bitmap *bm, g3s_phandle p) {
    ...
    fix dx, dy;
#if defined(__3DS__) || defined(_3DS)
    int citadel_native_bitmap_action = CITADEL_NATIVE_CAPTURE_SOFTWARE;
#endif
...
#if defined(__3DS__) || defined(_3DS)
    /*
     * R3D5B1A: attempt native capture before the legacy large-bitmap
     * scale/filter branch can submit software and return early.
     */
    citadel_native_bitmap_action =
        citadel_native_world_capture_bitmap(
            bm,
            _g3d_bitmap_poly,
            p->gZ,
            p->i,
    ...
    if (citadel_native_bitmap_action ==
            CITADEL_NATIVE_CAPTURE_BITMAP_ALPHA_ERASE) {
        if (citadel_native_world_apply_transparent_bitmap_erase(
                4,
                _g3d_bitmap_poly,
                &tmap_info)) {
        ...
        citadel_native_world_cancel_bitmap_capture();
    }
#endif
    // do legacy large-bitmap scaling/filtering?
