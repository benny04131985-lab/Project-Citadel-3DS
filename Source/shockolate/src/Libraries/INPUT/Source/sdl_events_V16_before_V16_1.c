...
#warning "PROJECT CITADEL INPUT V16: A/R swap is ACTIVE"
...
static void citadel_input_log(const char *fmt, ...)
{
    ...
    if (!citadel_input_log_initialized) {
        remove("INPUT_3DS_V16.log");
        citadel_input_log_initialized = true;
    }

    file = fopen("INPUT_3DS_V16.log", "a");
    if (file == NULL)
        return;
...
void pump_events(void) {
    ...
    if (!citadel_input_started) {
        ...
        citadel_set_mouse_position(citadel_current_mouse_width() / 2,
                                   ...
                                   false);
        citadel_input_log("PROJECT CITADEL INPUT V16 START build=%s %s mouse=%d,%d",
                          __DATE__,
                          __TIME__,
                          MouseX,
                          MouseY);
        citadel_input_log("CONTROLS circle=direct-analog cstick=panel-cursor A=native-freelook "
                          "touch=direct-lower R=left B=right X=space Y=enter "
