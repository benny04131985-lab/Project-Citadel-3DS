...
#warning "PROJECT CITADEL INPUT V9: polished movement and C-stick speed are ACTIVE"
...
static bool citadel_a_left_down = false;
static bool citadel_touch_left_down = false;
static bool citadel_b_right_down = false;
...
static void citadel_input_log(const char *fmt, ...)
{
    ...
    if (!citadel_input_log_initialized) {
        remove("INPUT_3DS_V9.log");
        citadel_input_log_initialized = true;
    }

    file = fopen("INPUT_3DS_V9.log", "a");
    if (file == NULL)
        return;
...
static void citadel_update_left_button(void)
{
    ...
    bool should_be_down = citadel_a_left_down || citadel_touch_left_down;
...
{
    static bool emitted_down = false;
    bool should_be_down = citadel_b_right_down;
...
static void citadel_process_3ds_input(u32 keys_down, u32 keys_up, u32 keys_held)
{
    ...
    if ((keys_down&KEY_R) && gameplay_active) { citadel_freelook_desired=!citadel_freelook_desired; citadel_input_log("FREELOOK desired=%d panel=%d local=%d,%d logical=%d,%d",citadel_freelook_desired,citadel_cursor_panel,citadel_cursor_local_x,citadel_cursor_local_y,MouseX,MouseY); }
    ...
    if (keys_down&KEY_A) { citadel_a_left_down=true; citadel_update_left_button(); }
    if (keys_up&KEY_A) { citadel_a_left_down=false; citadel_update_left_button(); }
    ...
    if (keys_down&KEY_B) { citadel_b_right_down=true; citadel_update_right_button(); }
    if (keys_up&KEY_B) { citadel_b_right_down=false; citadel_update_right_button(); }
...
void pump_events(void) {
    ...
    if (!citadel_input_started) {
        ...
        citadel_set_mouse_position(citadel_current_mouse_width() / 2,
                                   ...
                                   false);
        citadel_input_log("PROJECT CITADEL INPUT V9 START build=%s %s mouse=%d,%d",
                          __DATE__,
                          __TIME__,
                          MouseX,
                          MouseY);
        citadel_input_log("CONTROLS circle=direct-analog cstick=panel-cursor R=native-freelook "
                          "touch=direct-lower A=left B=right X=space Y=enter "
    ...
    while (SDL_PollEvent(&ev)) {
        ...
        case SDL_QUIT:
             ...
             */
            citadel_input_log("SDL_QUIT received; requesting clean main-loop exit");
            gPlayingGame = false;
            return;
#else
...
#if defined(__3DS__) || defined(_3DS)
    /*
     * The 3DS has no desktop relative-mouse stream. While R-toggle freelook
