    ...
    const citadel_c3d_profile_bucket *bucket)
{
    ...
    v5_log("GPU C3D R1 PROFILE mode=%s "
           "avg_palette_ms=%0.3f avg_left_directtiled_ms=%0.3f "
           "avg_left_fallback_swizzle_ms=%0.3f avg_right_acquire_ms=%0.3f "
           "avg_right_directtiled_ms=%0.3f avg_right_fallback_swizzle_ms=%0.3f",
           mode,
           citadel_c3d_profile_average_ms(
...
#define CITADEL_C3D_PROFILE_LOG_PATH \
    "sdmc:/3ds/SystemShock3D/C3D_R2_DIRECTTILEDPROFILE.log"
...
static bool citadel_gpu_upload_surface(SDL_Surface *surface)
{
    ...
    if (citadel_gpu_directtiled_enabled) {
        profile_tick = svcGetSystemTick();
...
void InitSDL() {
    ...
    v5_log("PROJECT CITADEL C3D R2A DIRECTTILED1 ACTIVE | "
