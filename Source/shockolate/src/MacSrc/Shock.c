...
#include "citadel_worldproof_vshader_shbin.h"
#include "citadel_worldtextured_vshader_shbin.h"
#include <unistd.h>
...
#warning "PROJECT CITADEL AUDIO SUSPEND HOTFIX V2: NDSP close/deferred reopen hook is ACTIVE"
#warning "PROJECT CITADEL 3D S1: zero-parallax dual-eye transport is ACTIVE"
#warning "PROJECT CITADEL 3D S2: true dual-camera world stereo is ACTIVE"
...
#warning "PROJECT CITADEL C3D R3A WORLDPROOF1: native polygon mirror overlay is ACTIVE"
#warning "PROJECT CITADEL C3D R3A2 WORLDSPACE SOLIDROOM1: camera-space terrain mirror is ACTIVE"
#warning "PROJECT CITADEL C3D R3B1 NATIVE STEREO ROUTE1: explicit engine eye routing is ACTIVE"
#warning "PROJECT CITADEL C3D R3B2 MONO TERRAIN TAKEOVER1: capture-success software terrain suppression is ACTIVE"
#warning "PROJECT CITADEL C3D R3C TEXTURED TERRAIN1: native UV/texture/light transport is ACTIVE"
#warning "PROJECT CITADEL C3D R3D1 KEYED FOREGROUND COMPOSITE1: software foreground replay is ACTIVE"
#warning "PROJECT CITADEL C3D R3D2B BITMAP SNAPSHOT1: immutable terrain bitmap identity + stereo native guard is ACTIVE"
#warning "PROJECT CITADEL C3D R3D5B2D NATIVE TEXBITMAP1: eligible non-door BIGSTUFF/SMALLSTUFF/FIXTURE TEXBITMAP objects use native oriented textured quads"
#warning "PROJECT CITADEL C3D R3D6B1 NATIVE PRESENTATION CUTOVER1: redundant top-eye RGB565 software transport is bypassed during proven native takeover"
#warning "PROJECT CITADEL C3D R3D6B2 NATIVE FOREGROUND DIRECT1: right residual + shared overlay fusion replaces full software right-frame composition during native stereo"
#warning "PROJECT CITADEL C3D R3D7C CMDBUF512 SHIPRC1: R3D7B renderer preserved with 512 KiB Citro3D command-buffer headroom"
...
/*
 * V16.1 lifecycle state:
 * - system_close_requested distinguishes HOME Menu "Close Software" from
 *   Shock's own in-game Quit command;
 * - the working-directory result is logged once SDL is available;
 * - SDL shutdown is guarded so explicit cleanup and atexit can coexist.
 */
bool citadel_3ds_system_close_requested = false;
static int citadel_3ds_working_directory_result = 0;
static bool citadel_sdl_shutdown_complete = false;
/*
 * V16.1 audio lifecycle hook. libctru invokes APTHOOK_ONSUSPEND before
 * saving/capturing the screens and before yielding DSP ownership to HOME.
 */
...
/* R3D1 keyed software-foreground texture: RGBA5551, alpha in bit zero. */
#define CITADEL_GPU_FOREGROUND_KEY_INDEX 0u
#define CITADEL_GPU_FOREGROUND_TEXTURE_BYTES \
...
static bool citadel_gpu_right_texture_initialized = false;
...
static C3D_Tex citadel_gpu_right_texture;
static u16 *citadel_gpu_staging = NULL;
static bool citadel_gpu_right_frame_active = false;
static float citadel_gpu_right_frame_slider = 0.0f;
static bool citadel_gpu_first_right_upload_logged = false;
...
/* PROJECT CITADEL C3D R3D6B1 NATIVE PRESENTATION CUTOVER1 */
static uint64_t citadel_r3d6b1_cutover_frames = 0;
static uint64_t citadel_r3d6b1_top_left_uploads_skipped = 0;
static uint64_t citadel_r3d6b1_top_right_uploads_skipped = 0;
static uint64_t citadel_r3d6b1_top_left_draws_skipped = 0;
static uint64_t citadel_r3d6b1_top_right_draws_skipped = 0;
static uint64_t citadel_r3d6b1_takeover_software_top_fallback_draws = 0;
static uint64_t citadel_r3d6b1_exceptional_software_top_frames = 0;

/* PROJECT CITADEL C3D R3D6B2 NATIVE FOREGROUND DIRECT1 */
static uint64_t citadel_r3d6b2_overlay_pixels_fused = 0;
static uint64_t citadel_r3d6b2_right_residual_pixels_sampled = 0;
...
/*
 * R3D1 follows the proven R2C ping-pong ownership rule: the foreground for
 * this frame is written into the same inactive set selected for the cropped
 * software frame, then sampled only after FrameBegin drains the prior set.
 */
static C3D_Tex citadel_gpu_foreground_texture_sets[2];
static C3D_Tex citadel_gpu_foreground_right_texture_sets[2];
static C3D_Tex *citadel_gpu_foreground_texture_live =
    &citadel_gpu_foreground_texture_sets[0];
static C3D_Tex *citadel_gpu_foreground_right_texture_live =
    &citadel_gpu_foreground_right_texture_sets[0];
static bool citadel_gpu_foreground_texture_initialized[2] = {false, false};
static bool citadel_gpu_foreground_right_texture_initialized[2] =
    {false, false};
static bool citadel_gpu_foreground_enabled = false;
static bool citadel_gpu_foreground_ready_this_frame = false;
static bool citadel_gpu_foreground_right_ready_this_frame = false;
static uint64_t citadel_gpu_foreground_frames_built = 0;
static uint64_t citadel_gpu_foreground_right_frames_built = 0;
static uint64_t citadel_gpu_foreground_left_draws = 0;
static uint64_t citadel_gpu_foreground_right_draws = 0;

/* R3D7A zero-disparity physical-right parity proof counters. */
static uint64_t citadel_r3d7a_parity_right_native_draw_calls = 0;
static uint64_t citadel_r3d7a_parity_right_native_triangles = 0;
static uint64_t citadel_r3d7a_parity_right_foreground_draws = 0;
static uint64_t citadel_r3d7a_right_software_frame_activations = 0;

/*
 * PROJECT CITADEL C3D R3D7B AVP DEPTH WARP1.
 *
 * R3D7A proved that one Shock traversal can feed both physical eyes without
 * the old right software frame/compositor. R3D7B keeps that exact ownership
 * and applies disparity only while re-emitting the SAME captured LEFT native
 * triangles to the physical RIGHT target.
 *
 * Citadel's exact-projected vertices preserve real camera depth in clip W:
 *     position = { screen_x * W, screen_y * W, 0, W }
 *
 * Therefore the right-eye pixel warp can use bounded reciprocal depth:
 *     shift_px = -strength * (1 / max(W, nearClamp) - 1 / convergenceW)
 *
 * Full-slider proof tuning:
 *     near clamp       0.25 camera units
 *     convergence      2.00 camera units
 *     scale            2.20 px per reciprocal-depth unit
 *     maximum shift    8.00 px
 *
 * This yields about -7.7 px at/inside the near clamp, zero at W=2.0, and
 * roughly +0.5 px at the historically observed ~3.6-4.0 far range.
 * The flat keyed foreground/HUD never enters this warp.
 */
#define CITADEL_R3D7B_STEREO_NEAR_CLAMP_W      0.25f
#define CITADEL_R3D7B_STEREO_CONVERGENCE_W     2.00f
#define CITADEL_R3D7B_STEREO_MAX_RECIP_SCALE   2.20f
#define CITADEL_R3D7B_STEREO_MAX_SHIFT_PIXELS  8.00f

/*
 * PROJECT CITADEL C3D R3D7C CMDBUF512 SHIPRC1.
 *
 * AvP/JK stereo testing proved that Citro3D command-buffer exhaustion can
 * surface only in later heavy scenes. Citadel R3D7B was already clean, so
 * this is release headroom rather than a renderer workaround.
 */
#define CITADEL_R3D7C_CMDBUF_SIZE 0x80000u

static bool citadel_r3d7b_right_warp_active = false;
static float citadel_r3d7b_right_warp_strength = 0.0f;
static uint64_t citadel_r3d7b_right_warp_draws = 0;
static uint64_t citadel_r3d7b_right_warp_vertices = 0;
static uint64_t citadel_r3d7b_zero_slider_right_draws = 0;
static bool citadel_r3d7b_warp_range_valid = false;
static float citadel_r3d7b_min_source_w = 0.0f;
static float citadel_r3d7b_max_source_w = 0.0f;
static float citadel_r3d7b_min_shift_pixels = 0.0f;
static float citadel_r3d7b_max_shift_pixels = 0.0f;
static float citadel_r3d7b_max_slider_seen = 0.0f;
static uint64_t citadel_gpu_foreground_opaque_texels = 0;
static uint64_t citadel_gpu_foreground_keyed_texels = 0;
static uint64_t citadel_gpu_foreground_right_opaque_texels = 0;
static uint64_t citadel_gpu_foreground_right_keyed_texels = 0;
static uint64_t citadel_gpu_foreground_build_failures = 0;
static uint64_t citadel_gpu_foreground_right_build_failures = 0;
static uint64_t citadel_gpu_foreground_draw_failures = 0;
static uint64_t citadel_gpu_foreground_reference_unavailable_frames = 0;
static uint64_t citadel_gpu_foreground_reference_mismatch_texels = 0;
static uint64_t citadel_gpu_foreground_reference_match_texels = 0;
static uint64_t citadel_gpu_foreground_reference_direct_samples = 0;
static uint64_t citadel_gpu_foreground_reference_scaled_samples = 0;
static uint64_t citadel_gpu_foreground_reference_coordinate_failures = 0;
static int citadel_gpu_foreground_reference_last_w = 0;
static int citadel_gpu_foreground_reference_last_h = 0;
static int citadel_gpu_foreground_reference_last_source_x = 0;
static int citadel_gpu_foreground_reference_last_source_y = 0;
static int citadel_gpu_foreground_reference_last_source_w = 0;
static int citadel_gpu_foreground_reference_last_source_h = 0;
...
/*
 * R3D6B1 only cuts the redundant top RGB565 transport when the exact
 * native foreground-present path is healthy and the cropped resources
 * needed for the bottom screen and keyed foreground remain available.
 */
static bool citadel_r3d6b1_cutover_candidate(bool split_layout)
{
    return split_layout &&
           citadel_native_world_foreground_present_active() &&
...
#define CITADEL_DIAG_VERSION "1.0.3-DIAG4-UNIQUE-RUN"
#define CITADEL_DIAG_HISTOGRAM_MAX_MS 250
#define CITADEL_DIAG_LONG_GAP_MS 1000.0

#define CITADEL_RUN_EVIDENCE_DIR \
    "sdmc:/3ds/SystemShock3D"
#define CITADEL_RUN_EVIDENCE_TAG \
    "R3D7C"

static bool citadel_run_evidence_ready = false;
static unsigned long long citadel_run_evidence_epoch = 0;
static unsigned long long citadel_run_evidence_tick = 0;
static char citadel_run_evidence_token[64];
static char citadel_run_started_path[256];
static char citadel_run_diag_path[256];
static char citadel_run_diag_fallback_path[256];
static char citadel_run_worldproof_path[256];

#define CITADEL_DIAG_PATH \
    citadel_run_diag_path
#define CITADEL_DIAG_FALLBACK_PATH \
    citadel_run_diag_fallback_path

static void citadel_run_evidence_prepare(void)
{
    FILE *marker;
    time_t now;
    u64 tick;

    if (citadel_run_evidence_ready)
        return;

    now = time(NULL);
    tick = svcGetSystemTick();

    citadel_run_evidence_epoch =
        (unsigned long long)now;
    citadel_run_evidence_tick =
        (unsigned long long)tick;

    snprintf(
        citadel_run_evidence_token,
        sizeof(citadel_run_evidence_token),
        "%llu_%016llX",
        citadel_run_evidence_epoch,
        citadel_run_evidence_tick);

    snprintf(
        citadel_run_started_path,
        sizeof(citadel_run_started_path),
        CITADEL_RUN_EVIDENCE_DIR
        "/C3D_RUN_" CITADEL_RUN_EVIDENCE_TAG
        "_%s_STARTED.txt",
        citadel_run_evidence_token);

    snprintf(
        citadel_run_diag_path,
        sizeof(citadel_run_diag_path),
        CITADEL_RUN_EVIDENCE_DIR
        "/C3D_RUN_" CITADEL_RUN_EVIDENCE_TAG
        "_%s_DIAG.log",
        citadel_run_evidence_token);
    ...
    snprintf(
        citadel_run_diag_fallback_path,
        sizeof(citadel_run_diag_fallback_path),
        CITADEL_RUN_EVIDENCE_DIR
        "/C3D_RUN_" CITADEL_RUN_EVIDENCE_TAG
        "_%s_DIAG_FALLBACK.log",
        citadel_run_evidence_token);

    snprintf(
        citadel_run_worldproof_path,
        sizeof(citadel_run_worldproof_path),
        CITADEL_RUN_EVIDENCE_DIR
        "/C3D_RUN_" CITADEL_RUN_EVIDENCE_TAG
        "_%s_DEPTH_PROOF.log",
        citadel_run_evidence_token);

    marker = fopen(citadel_run_started_path, "wb");

    if (marker != NULL) {
        fprintf(marker,
                "PROJECT CITADEL UNIQUE RUN EVIDENCE\n"
                "Logger milestone: CITADEL-C3D-R3D7C-CMDBUF512-SHIPRC1\n"
                "Renderer milestone: CITADEL-C3D-R3D7C-CMDBUF512-SHIPRC1\n"
                "Build: %s %s\n"
                "Run token: %s\n"
                "Session epoch: %llu\n"
                "Hardware tick: 0x%016llX\n"
                "Expected proof log: %s\n"
                "Expected diag log: %s\n",
                __DATE__,
                __TIME__,
                citadel_run_evidence_token,
    ...
    citadel_run_evidence_ready = true;

    v5_log(
        "CITADEL UNIQUE RUN token=%s marker=%s proof=%s diag=%s",
        citadel_run_evidence_token,
        citadel_run_started_path,
        citadel_run_worldproof_path,
        citadel_run_diag_path);
...
static const char *citadel_diag_path_used = NULL;
static int citadel_diag_primary_remove_result = 0;
static int citadel_diag_primary_remove_errno = 0;
static int citadel_diag_primary_open_errno = 0;
static int citadel_diag_fallback_open_errno = 0;
static int citadel_diag_primary_open_success = 0;
static int citadel_diag_fallback_open_success = 0;
...
/* R3D7C release headroom telemetry. C3D reports usage as a 0..1 fraction. */
static float citadel_r3d7c_cmd_buf_peak_usage = 0.0f;
static float citadel_r3d7c_cmd_buf_last_usage = 0.0f;
static unsigned long citadel_r3d7c_cmd_buf_samples = 0;
...
static void citadel_diag_init(void)
{
    ...
    errno = 0;
    citadel_diag_primary_remove_result =
        remove(CITADEL_DIAG_PATH);
    citadel_diag_primary_remove_errno = errno;

    errno = 0;
    citadel_diag_file = fopen(CITADEL_DIAG_PATH, "wb");
    citadel_diag_primary_open_errno = errno;

    if (citadel_diag_file != NULL) {
        citadel_diag_primary_open_success = 1;
        citadel_diag_path_used = CITADEL_DIAG_PATH;
    } else {
        /*
         * The original DIAG2 logger silently returned when fopen failed,
         * leaving an old file that looked current. Use a second filename so
         * a stale/read-only/cached canonical entry cannot hide the new run.
         */
        (void)remove(CITADEL_DIAG_FALLBACK_PATH);
        errno = 0;
        citadel_diag_file =
            fopen(CITADEL_DIAG_FALLBACK_PATH, "wb");
        citadel_diag_fallback_open_errno = errno;

        if (citadel_diag_file != NULL) {
            citadel_diag_fallback_open_success = 1;
            citadel_diag_path_used = CITADEL_DIAG_FALLBACK_PATH;
        }
    }

    v5_log(
        "CITADEL DIAG3 OPEN primary_remove=%d remove_errno=%d "
        "primary_open=%s primary_errno=%d fallback_open=%s "
        "fallback_errno=%d path=%s",
        citadel_diag_primary_remove_result,
        citadel_diag_primary_remove_errno,
        citadel_diag_primary_open_success ? "OK" : "FAIL",
        citadel_diag_primary_open_errno,
        citadel_diag_fallback_open_success ? "OK" : "NOT_USED_OR_FAIL",
        citadel_diag_fallback_open_errno,
        citadel_diag_path_used != NULL
            ? citadel_diag_path_used
            : "NONE");
    ...
    fprintf(citadel_diag_file,
            ...
            citadel_diag_path_used != NULL
                ...
                : "UNKNOWN");
    fprintf(citadel_diag_file,
            "Primary remove result: %d errno=%d\n",
            citadel_diag_primary_remove_result,
            citadel_diag_primary_remove_errno);
    fprintf(citadel_diag_file,
            "Primary open: %s errno=%d\n",
            citadel_diag_primary_open_success ? "YES" : "NO",
            citadel_diag_primary_open_errno);
    fprintf(citadel_diag_file,
            "Fallback open: %s errno=%d\n",
            citadel_diag_fallback_open_success ? "YES" : "NO",
            citadel_diag_fallback_open_errno);
    fprintf(citadel_diag_file,
    ...
    fprintf(citadel_diag_file,
            ...
            citadel_diag_initial_linear_free);
    fprintf(citadel_diag_file,
            "Citro3D command buffer: %u bytes (512 KiB ship headroom)\n",
            (unsigned int)CITADEL_R3D7C_CMDBUF_SIZE);
    fprintf(citadel_diag_file,
            "Command-buffer usage telemetry: C3D_GetCmdBufUsage EACH COMPLETED FRAME\n");
    fprintf(citadel_diag_file,
...
static void citadel_diag_note_frame(void)
{
    ...
    /*
     * R3D7C: C3D_GetCmdBufUsage() is only a context-field read. Sampling it
     * here, immediately after completed presentations call this function,
     * adds no GPU synchronization and gives release-test peak headroom.
     */
    {
        const float usage = C3D_GetCmdBufUsage();

        citadel_r3d7c_cmd_buf_last_usage = usage;
        ++citadel_r3d7c_cmd_buf_samples;

        if (usage > citadel_r3d7c_cmd_buf_peak_usage)
            citadel_r3d7c_cmd_buf_peak_usage = usage;
    }
...
static void citadel_diag_shutdown(void)
{
    ...
    fprintf(citadel_diag_file,
            ...
            citadel_diag_lowest_linear_free);
    fprintf(citadel_diag_file,
            "Citro3D command buffer size: %u bytes\n",
            (unsigned int)CITADEL_R3D7C_CMDBUF_SIZE);
    fprintf(citadel_diag_file,
            "Command-buffer usage samples: %lu\n",
            citadel_r3d7c_cmd_buf_samples);
    fprintf(citadel_diag_file,
            "Command-buffer last usage: %.3f %%\n",
            (double)citadel_r3d7c_cmd_buf_last_usage * 100.0);
    fprintf(citadel_diag_file,
            "Command-buffer peak usage: %.3f %%\n",
            (double)citadel_r3d7c_cmd_buf_peak_usage * 100.0);
    fprintf(citadel_diag_file,
            "Command-buffer peak estimated bytes: %u\n",
            (unsigned int)(
                citadel_r3d7c_cmd_buf_peak_usage *
                (float)CITADEL_R3D7C_CMDBUF_SIZE));
    fprintf(citadel_diag_file,
...
extern const unsigned char *citadel_3ds_stereo_get_right_frame(
    int *width, int *height, int *pitch, float *slider);
extern int citadel_3ds_stereo_direct_foreground_active(void);
extern const unsigned char *citadel_3ds_stereo_get_left_world_baseline(
    int *width, int *height, int *pitch);
extern int citadel_3ds_stereo_get_convergence_shift(void);
extern int citadel_3ds_stereo_force_legacy_compositor(void);
extern int citadel_3ds_stereo_frame_enabled(void);
extern unsigned long long citadel_r3d7a_stereo_frames;
extern unsigned long long citadel_r3d7a_scene_traversals;
extern unsigned long long citadel_r3d7a_right_traversals_skipped;
extern unsigned long long citadel_r3d7a_left_parity_passes;
extern unsigned long long citadel_r3d7a_left_capture_bypassed;
extern unsigned long long citadel_r3d7a_legacy_compose_bypassed;
extern unsigned long long citadel_r3d7a_unexpected_right_passes;

extern unsigned long long citadel_r3d6b2_direct_foreground_frames;
extern unsigned long long citadel_r3d6b2_right_residual_captures;
extern unsigned long long citadel_r3d6b2_left_foreground_row_captures;
extern unsigned long long citadel_r3d6b2_left_full_capture_bypassed;
extern unsigned long long citadel_r3d6b2_compose_scans_bypassed;
extern unsigned long long citadel_r3d6b2_legacy_compositor_frames;
extern unsigned long long citadel_r3d6b2_direct_fallbacks;
extern unsigned long long citadel_r3d6b2_foreground_capture_bytes;
extern unsigned long long citadel_r3d6b3_direct_right_buffer_frames;
extern unsigned long long citadel_r3d6b3_right_full_capture_bypassed;
extern unsigned long long citadel_r3d6b3_right_memcpy_fallback_captures;
extern unsigned long long citadel_r3d6b3_right_memcpy_bytes_saved;
extern unsigned long long citadel_r3d6b3_screen_restore_failures;
...
static void citadel_sdl_shutdown_once(void);
extern void citadel_3ds_audio_suspend(void);
extern void citadel_3ds_audio_resume(void);
extern void citadel_3ds_audio_service_resume(void);
...
static bool citadel_gpu_foreground_initialize(void);
static void citadel_gpu_foreground_shutdown(void);
    ...
    void *parameter)
{
    ...
    switch (hook) {
        case APTHOOK_ONSUSPEND:
            ...
            citadel_3ds_audio_suspend();
            v5_log("APT AUDIO HOOK event=ONSUSPEND complete");
            break;

        case APTHOOK_ONSLEEP:
            v5_log("APT AUDIO HOOK event=ONSLEEP begin");
            citadel_3ds_audio_suspend();
            v5_log("APT AUDIO HOOK event=ONSLEEP complete");
            break;
...
static void citadel_3ds_audio_register_apt_hook(void)
{
    if (citadel_3ds_audio_apt_hook_registered)
        return;

    aptHook(
        &citadel_3ds_audio_apt_cookie,
        citadel_3ds_audio_apt_hook,
        NULL);

    citadel_3ds_audio_apt_hook_registered = true;
    v5_log("APT AUDIO HOOK registered");
}
...
static void citadel_3ds_audio_unregister_apt_hook(void)
{
    ...
    aptUnhook(&citadel_3ds_audio_apt_cookie);
    citadel_3ds_audio_apt_hook_registered = false;
    v5_log("APT AUDIO HOOK unregistered");
}

static void citadel_sdl_shutdown_once(void)
{
    if (citadel_sdl_shutdown_complete)
        return;

    citadel_sdl_shutdown_complete = true;
    SDL_Quit();
}
...
/* -------------------------------------------------------------------------
 * PROJECT CITADEL C3D R3D1 KEYED FOREGROUND COMPOSITE1
 *
 * R3C native textured terrain remains intact. R3D1 rebuilds the software
 * foreground as an RGBA5551 keyed layer and draws it after native terrain.
 *
 * BASE: PROJECT CITADEL C3D R3C TEXTURED TERRAIN1
 *
 * R3B2 capture-success mono terrain takeover remains the ownership policy.
 * R3C adds genuine indexed terrain texture transport, UV coordinates, basic
 * vertex-light modulation, a bounded LRU Citro3D cache, and flat-color GPU
 * fallback if a texture cannot be uploaded at presentation time.
 * ------------------------------------------------------------------------- */
#define CITADEL_NATIVE_WORLDPROOF_LOG \
    citadel_run_worldproof_path

#define CITADEL_NATIVE_TEXTURE_CACHE_CAPACITY 128u

typedef struct CitadelNativeTextureCacheEntry {
    C3D_Tex texture;
    const void *bits_key;
    int width;
    int height;
    ...
    uint64_t last_used;
    bool initialized;
} CitadelNativeTextureCacheEntry;

static bool citadel_native_worldproof_shader_ready = false;
static bool citadel_native_worldproof_program_initialized = false;
static bool citadel_native_worldtextured_shader_ready = false;
static bool citadel_native_worldtextured_program_initialized = false;
static bool citadel_native_worldproof_first_left_draw_logged = false;
static bool citadel_native_worldproof_first_right_draw_logged = false;

static DVLB_s *citadel_native_worldproof_dvlb = NULL;
static shaderProgram_s citadel_native_worldproof_program;
static DVLB_s *citadel_native_worldtextured_dvlb = NULL;
static shaderProgram_s citadel_native_worldtextured_program;

static int citadel_native_worldproof_projection_uniform = -1;
static int citadel_native_worldproof_model_view_uniform = -1;
static int citadel_native_worldtextured_projection_uniform = -1;
static int citadel_native_worldtextured_model_view_uniform = -1;

static C3D_Mtx citadel_native_worldproof_projection;
static C3D_Mtx citadel_native_worldproof_model_view;

static int citadel_native_projection_last_source_w = 0;
static int citadel_native_projection_last_source_h = 0;
static int citadel_native_projection_last_destination_w = 0;
static int citadel_native_projection_last_destination_h = 0;
static uint64_t citadel_native_projected_vertices_sent = 0;

static CitadelNativeTextureCacheEntry
    citadel_native_texture_cache[CITADEL_NATIVE_TEXTURE_CACHE_CAPACITY];
static uint32_t citadel_native_texture_palette_generation = 1u;
static uint64_t citadel_native_texture_serial = 0;
static uint64_t citadel_native_texture_cache_hits = 0;
static uint64_t citadel_native_texture_cache_misses = 0;
static uint64_t citadel_native_texture_uploads = 0;
static uint64_t citadel_native_texture_refreshes = 0;
static uint64_t citadel_native_texture_evictions = 0;
static uint64_t citadel_native_texture_upload_failures = 0;
static uint64_t citadel_native_alpha_texture_cache_hits = 0;
static uint64_t citadel_native_alpha_texture_uploads = 0;
static uint64_t citadel_native_textured_triangles_drawn = 0;
static uint64_t citadel_native_door_textured_triangles_drawn = 0;
static uint64_t citadel_native_alpha_door_textured_triangles_drawn = 0;
static uint64_t citadel_native_world_bitmap_textured_triangles_drawn = 0;
static uint64_t citadel_native_critter_bitmap_textured_triangles_drawn = 0;
static uint64_t citadel_native_multiview_bitmap_textured_triangles_drawn = 0;
static uint64_t citadel_native_texbitmap_textured_triangles_drawn = 0;
static uint64_t citadel_native_flat_fallback_triangles_drawn = 0;
static uint64_t citadel_native_texture_draw_runs = 0;
static uint64_t citadel_native_stereo_overlay_skips = 0;

static inline unsigned int citadel_native_texture_morton8(
    unsigned int x,
    unsigned int y)
{
    ...
    return (unsigned int)morton_x[x & 7u] +
           (unsigned int)morton_y[y & 7u];
}

static inline size_t citadel_native_texture_tiled_offset(
    unsigned int width,
    unsigned int x,
    unsigned int y)
{
    const unsigned int tiles_per_row = width / 8u;
    ...
    return tile_base + (size_t)citadel_native_texture_morton8(x, y);
}

static inline u16 citadel_native_texture_color_to_rgb565(SDL_Color color)
{
...
static inline u16 citadel_native_texture_color_to_rgba5551(
    SDL_Color color,
    int opaque)
{
    const u16 r5 = (u16)(color.r >> 3);
    const u16 g5 = (u16)(color.g >> 3);
    const u16 b5 = (u16)(color.b >> 3);
...
static void citadel_native_texture_note_palette_changed(void)
{
    ++citadel_native_texture_palette_generation;

    if (citadel_native_texture_palette_generation == 0)
        citadel_native_texture_palette_generation = 1u;
}

static void citadel_native_texture_release_entry(
    CitadelNativeTextureCacheEntry *entry)
{
    if (entry == NULL)
        return;
...
static void citadel_native_texture_cache_shutdown(void)
{
    unsigned int index;

    for (index = 0;
         index < CITADEL_NATIVE_TEXTURE_CACHE_CAPACITY;
         ++index) {
        citadel_native_texture_release_entry(
            &citadel_native_texture_cache[index]);
    }
}

static bool citadel_native_texture_entry_matches(
    const CitadelNativeTextureCacheEntry *entry,
    ...
    uint64_t content_key)
{
    return entry != NULL &&
           bitmap != NULL &&
           entry->initialized &&
           entry->bits_key == bitmap->bits &&
           entry->width == bitmap->w &&
           entry->height == bitmap->h &&
           entry->row == bitmap->row &&
           entry->type == bitmap->type &&
           entry->flags == (unsigned int)bitmap->flags &&
           entry->texture_format ==
               ((bitmap->flags & BMF_TRANS) ? GPU_RGBA5551 : GPU_RGB565) &&
           entry->content_key == content_key;
}

static bool citadel_native_texture_upload(
    CitadelNativeTextureCacheEntry *entry,
    const grs_bitmap *bitmap)
{
    ...
    if (entry == NULL || bitmap == NULL ||
        !entry->initialized || entry->texture.data == NULL)
        return false;

    source_pixels = (const u8 *)bitmap->bits;
    source_pitch = bitmap->row;

    if (bitmap->type == BMT_RSD8) {
        memset(&decoded, 0, sizeof(decoded));

        if (gr_rsd8_convert((grs_bitmap *)bitmap, &decoded) != 0 ||
            decoded.bits == NULL) {
            return false;
        }

        source_pixels = (const u8 *)decoded.bits;
        source_pitch = decoded.row >= (ushort)bitmap->w
            ? decoded.row
            : bitmap->w;
    }

    if (source_pixels == NULL || source_pitch < bitmap->w)
        return false;

    destination_pixels = (u16 *)entry->texture.data;
    memset(
        destination_pixels,
        0,
        (size_t)bitmap->w * (size_t)bitmap->h * sizeof(u16));

    /*
     * Shock/OpenGL treats UV v=0 as the source image's first/top row. PICA's
     * sampled texture orientation is opposite the proven top-origin upload,
     * so flip source rows during Morton upload and keep Shock UVs unchanged.
     */
    for (destination_y = 0;
         destination_y < bitmap->h;
         ++destination_y) {
        const int source_y = bitmap->h - 1 - destination_y;
        const u8 *source_row =
            source_pixels + ((size_t)source_y * (size_t)source_pitch);
        int x;

        for (x = 0; x < bitmap->w; ++x) {
            const unsigned int palette_index = source_row[x];
            const SDL_Color color = sdlPalette != NULL
                ? sdlPalette->colors[palette_index]
                : gamePalette[palette_index];
            const size_t offset = citadel_native_texture_tiled_offset(
                (unsigned int)bitmap->w,
                (unsigned int)x,
                (unsigned int)destination_y);

            if (entry->texture_format == GPU_RGBA5551) {
                destination_pixels[offset] =
                    palette_index == 0
                        ? 0
                        : citadel_native_texture_color_to_rgba5551(
                            color,
                            1);
            } else {
                destination_pixels[offset] =
                    citadel_native_texture_color_to_rgb565(color);
            }
        }
    }
...
static CitadelNativeTextureCacheEntry *
citadel_native_texture_get(
    const grs_bitmap *bitmap,
    uint64_t content_key)
{
    CitadelNativeTextureCacheEntry *entry = NULL;
    CitadelNativeTextureCacheEntry *oldest = NULL;
    unsigned int index;

    if (bitmap == NULL || bitmap->bits == NULL)
        return NULL;

    ++citadel_native_texture_serial;

    for (index = 0;
         index < CITADEL_NATIVE_TEXTURE_CACHE_CAPACITY;
         ++index) {
        CitadelNativeTextureCacheEntry *candidate =
            &citadel_native_texture_cache[index];

        if (citadel_native_texture_entry_matches(
                candidate,
                bitmap,
                content_key)) {
            entry = candidate;
        ...
        if (!candidate->initialized && entry == NULL)
            entry = candidate;
    ...
    if (entry != NULL && entry->initialized) {
        ++citadel_native_texture_cache_hits;
        if (bitmap->flags & BMF_TRANS)
            ++citadel_native_alpha_texture_cache_hits;
        entry->last_used = citadel_native_texture_serial;

        if (entry->palette_generation !=
            citadel_native_texture_palette_generation) {
            if (!citadel_native_texture_upload(entry, bitmap)) {
                ++citadel_native_texture_upload_failures;
                citadel_native_texture_release_entry(entry);
                return NULL;
            }

            ++citadel_native_texture_refreshes;
        }

        return entry;
    }

    ++citadel_native_texture_cache_misses;
    ...
    if (entry->initialized) {
        ++citadel_native_texture_evictions;
        citadel_native_texture_release_entry(entry);
    }
    ...
    if (entry->texture_format == GPU_RGBA5551) {
        ...
        C3D_TexSetFilter(
            &entry->texture,
            gShockPrefs.doTextureFilter == 0 ? GPU_NEAREST : GPU_LINEAR,
            gShockPrefs.doTextureFilter == 0 ? GPU_NEAREST : GPU_LINEAR);
    }
    C3D_TexSetWrap(&entry->texture, GPU_REPEAT, GPU_REPEAT);
    ...
    ++citadel_native_texture_uploads;
    if (entry->texture_format == GPU_RGBA5551)
        ++citadel_native_alpha_texture_uploads;
    return entry;
}

static void citadel_native_worldproof_shutdown(void)
{
    citadel_native_texture_cache_shutdown();

    if (citadel_native_worldtextured_program_initialized) {
        shaderProgramFree(&citadel_native_worldtextured_program);
        citadel_native_worldtextured_program_initialized = false;
    }

    if (citadel_native_worldtextured_dvlb != NULL) {
        DVLB_Free(citadel_native_worldtextured_dvlb);
        citadel_native_worldtextured_dvlb = NULL;
    }

    if (citadel_native_worldproof_program_initialized) {
        shaderProgramFree(&citadel_native_worldproof_program);
        citadel_native_worldproof_program_initialized = false;
    }

    if (citadel_native_worldproof_dvlb != NULL) {
        DVLB_Free(citadel_native_worldproof_dvlb);
        citadel_native_worldproof_dvlb = NULL;
    }

    citadel_native_worldproof_projection_uniform = -1;
    citadel_native_worldproof_model_view_uniform = -1;
    citadel_native_worldtextured_projection_uniform = -1;
    citadel_native_worldtextured_model_view_uniform = -1;
    citadel_native_worldproof_shader_ready = false;
    citadel_native_worldtextured_shader_ready = false;
}

static bool citadel_native_worldproof_initialize(void)
{
    citadel_native_worldproof_dvlb = DVLB_ParseFile(
        (u32 *)citadel_worldproof_vshader_shbin,
        citadel_worldproof_vshader_shbin_size);

    if (citadel_native_worldproof_dvlb == NULL) {
        v5_log("GPU C3D R3C INIT FAIL stage=flat-DVLB_ParseFile");
        return false;
    }

    shaderProgramInit(&citadel_native_worldproof_program);
    citadel_native_worldproof_program_initialized = true;
    shaderProgramSetVsh(
        &citadel_native_worldproof_program,
        &citadel_native_worldproof_dvlb->DVLE[0]);

    citadel_native_worldproof_projection_uniform =
        shaderInstanceGetUniformLocation(
            citadel_native_worldproof_program.vertexShader,
            "projection");
    citadel_native_worldproof_model_view_uniform =
        shaderInstanceGetUniformLocation(
            citadel_native_worldproof_program.vertexShader,
            "modelView");

    if (citadel_native_worldproof_projection_uniform < 0 ||
        citadel_native_worldproof_model_view_uniform < 0) {
        v5_log("GPU C3D R3C INIT FAIL stage=flat-uniforms");
        citadel_native_worldproof_shutdown();
        return false;
    }

    citadel_native_worldtextured_dvlb = DVLB_ParseFile(
        (u32 *)citadel_worldtextured_vshader_shbin,
        citadel_worldtextured_vshader_shbin_size);

    if (citadel_native_worldtextured_dvlb != NULL) {
        shaderProgramInit(&citadel_native_worldtextured_program);
        citadel_native_worldtextured_program_initialized = true;
        shaderProgramSetVsh(
            &citadel_native_worldtextured_program,
            &citadel_native_worldtextured_dvlb->DVLE[0]);

        citadel_native_worldtextured_projection_uniform =
            shaderInstanceGetUniformLocation(
                citadel_native_worldtextured_program.vertexShader,
                "projection");
        citadel_native_worldtextured_model_view_uniform =
            shaderInstanceGetUniformLocation(
                citadel_native_worldtextured_program.vertexShader,
                "modelView");

        citadel_native_worldtextured_shader_ready =
            citadel_native_worldtextured_projection_uniform >= 0 &&
            citadel_native_worldtextured_model_view_uniform >= 0;
    }

    /*
     * R3D4D: positions already contain the software renderer's exact
     * projected pixels. The orthographic screen matrix places them without a
     * second camera/FOV approximation.
     */
    Mtx_OrthoTilt(
        &citadel_native_worldproof_projection,
        0.0f, 400.0f, 240.0f, 0.0f,
        1.0f, -1.0f, true);
    Mtx_Identity(&citadel_native_worldproof_model_view);

    citadel_native_worldproof_shader_ready = true;

    v5_log("GPU C3D R3D4D INIT SUCCESS flat_shader=%u textured_shader=%u "
           "projection=ORTHO_SCREEN_EXACT cache_capacity=%u",
           citadel_native_worldproof_shader_ready ? 1U : 0U,
           citadel_native_worldtextured_shader_ready ? 1U : 0U,
           (unsigned int)CITADEL_NATIVE_TEXTURE_CACHE_CAPACITY);

    return true;
}

static void citadel_native_worldproof_write_start(void)
{
    FILE *file;

    citadel_run_evidence_prepare();

    remove(CITADEL_NATIVE_WORLDPROOF_LOG);
    file = fopen(CITADEL_NATIVE_WORLDPROOF_LOG, "w");
    ...
    fprintf(file,
            ...
            "Output path: %s\n"
            "Build: %s %s\n"
            "Parent renderer: CITADEL-C3D-R3D7B-AVP-DEPTH-WARP1-HWPASS\n"
            "Architecture: AVP_STYLE_SINGLE_STREAM_IMMEDIATE_DUAL_TARGET\n"
            "Stereo scene traversal: ONE_FR_REND_ONLY\n"
            "Native capture policy: LEFT_CAPTURE_REUSED_FOR_PHYSICAL_RIGHT\n"
            "R3D7B disparity policy: RIGHT_ONLY_RECIPROCAL_W_DEPTH_WARP\n"
            "Warp formula: shift_px=-strength*(1/max(W,0.25)-1/2.0)\n"
            "Full-slider reciprocal scale: %.3f px\n"
            "Maximum absolute warp: %.3f px\n"
            "Citro3D command buffer: 524288 bytes (512 KiB)\n"
            "Command-buffer telemetry: ENABLED_PER_COMPLETED_FRAME\n"
            "Release role: V2.0.0_SHIP_CANDIDATE_HARDENING\n"
            "Physical right target: SAME_NATIVE_STREAM_AS_LEFT_WITH_DEPTH_WARP\n"
            "Right software residual: DISABLED\n"
            "Right framebuffer reconstruction: DISABLED\n"
            "Left software baseline capture: DISABLED\n"
            "Flat foreground policy: ONE_LEFT_TEXTURE_REUSED_BOTH_EYES_ZERO_DISPARITY\n"
            "HUD/overlay callback count: ONE_LEFT_PASS_ONLY\n"
            "Bottom-screen software transport: PRESERVED\n"
            "Next after proof: V2.0.0_SHIP_IF_MONO_STEREO_AND_HEADROOM_PASS\n"
            "Shutdown summary pending: YES\n"
            "============================================================\n",
            citadel_run_evidence_token,
            CITADEL_NATIVE_WORLDPROOF_LOG,
            __DATE__,
            __TIME__,
            (double)CITADEL_R3D7B_STEREO_MAX_RECIP_SCALE,
            (double)CITADEL_R3D7B_STEREO_MAX_SHIFT_PIXELS);

    fclose(file);
}

static void citadel_native_worldproof_write_summary(void)
{
    CitadelNativeWorldStats stats;
    FILE *file;
    double mono_fps = 0.0;
    double stereo_fps = 0.0;
    bool r3d7b_depth_proven;

    citadel_native_world_get_stats(&stats);

    if (citadel_diag_mono_total_frame_ms > 0.0)
        mono_fps =
            ((double)citadel_diag_mono_frames * 1000.0) /
            citadel_diag_mono_total_frame_ms;

    if (citadel_diag_stereo_total_frame_ms > 0.0)
        stereo_fps =
            ((double)citadel_diag_stereo_frames * 1000.0) /
            citadel_diag_stereo_total_frame_ms;

    r3d7b_depth_proven =
        citadel_r3d7a_stereo_frames > 0 &&
        citadel_r3d7a_scene_traversals == citadel_r3d7a_stereo_frames &&
        citadel_r3d7a_right_traversals_skipped == citadel_r3d7a_stereo_frames &&
        citadel_r3d7a_left_parity_passes > 0 &&
        citadel_r3d7a_left_capture_bypassed > 0 &&
        citadel_r3d7a_legacy_compose_bypassed > 0 &&
        citadel_r3d7a_unexpected_right_passes == 0 &&
        citadel_r3d7a_right_software_frame_activations == 0 &&
        stats.left_triangles_captured > 0 &&
        stats.right_triangles_captured == 0 &&
        citadel_r3d7a_parity_right_native_draw_calls > 0 &&
        citadel_r3d7a_parity_right_native_triangles > 0 &&
        citadel_r3d7b_right_warp_draws > 0 &&
        citadel_r3d7b_right_warp_vertices > 0 &&
        citadel_r3d7b_warp_range_valid &&
        citadel_gpu_foreground_frames_built > 0 &&
        citadel_gpu_foreground_right_frames_built == 0 &&
        citadel_gpu_foreground_left_draws > 0 &&
        citadel_r3d7a_parity_right_foreground_draws > 0 &&
        citadel_gpu_foreground_build_failures == 0 &&
        citadel_gpu_foreground_right_build_failures == 0 &&
        citadel_gpu_foreground_draw_failures == 0 &&
        citadel_native_texture_upload_failures == 0 &&
        stats.capture_overflows == 0 &&
        stats.draw_budget_drops == 0 &&
        citadel_gpu_upload_failures == 0 &&
        citadel_gpu_draw_failures == 0;

    file = fopen(CITADEL_NATIVE_WORLDPROOF_LOG, "a");
    if (file == NULL)
        return;
    ...
    fprintf(file,
            ...
            "Milestone: CITADEL-C3D-R3D7C-CMDBUF512-SHIPRC1\n"
            "Run token: %s\n"
            "R3D7C ship-candidate renderer path proven: %s\n"
            "Stereo frames scheduled: %llu\n"
            "Shock/fr_rend scene traversals: %llu\n"
            "Physical right engine traversals skipped: %llu\n"
            "R3D7A base left parity passes: %llu\n"
            "Left software baseline captures bypassed: %llu\n"
            "Legacy right compositor passes bypassed: %llu\n"
            "Unexpected old right engine passes: %llu\n"
            "Right software frame activations: %llu\n"
            "Native LEFT triangles captured: %llu\n"
            "Native RIGHT triangles captured: %llu\n"
            "Physical-right native draw calls: %llu\n"
            "Physical-right native triangles: %llu\n"
            "R3D7B right depth-warp draws: %llu\n"
            "R3D7B right depth-warp vertices: %llu\n"
            "R3D7B zero-slider right draws: %llu\n"
            "R3D7B maximum slider seen: %.4f\n"
            "R3D7B source W range: %.6f .. %.6f\n"
            "R3D7B right shift range pixels: %.6f .. %.6f\n"
            "R3D7B near clamp W: %.3f\n"
            "R3D7B convergence W: %.3f\n"
            "R3D7B full-slider reciprocal scale: %.3f\n"
            "R3D7B maximum absolute shift: %.3f\n"
            "Left foreground frames built: %llu\n"
            "Separate right foreground frames built: %llu\n"
            "Left foreground draws: %llu\n"
            "Physical-right FLAT foreground draws: %llu\n"
            "Foreground build failures: %llu\n"
            "Foreground right-eye build failures: %llu\n"
            "Foreground draw failures: %llu\n"
            "Capture overflows: %llu\n"
            "Draw-budget drops: %llu\n"
            "GPU texture upload failures: %llu\n"
            "GPU presentation upload failures: %u\n"
            "GPU draw failures: %u\n"
            "Citro3D command buffer size: %u bytes\n"
            "Command-buffer usage samples: %lu\n"
            "Command-buffer peak usage: %.3f %%\n"
            "Command-buffer peak estimated bytes: %u\n"
            "Mono measured frames: %lu\n"
            "Mono average FPS: %.3f\n"
            "Stereo measured frames: %lu\n"
            "Stereo average FPS: %.3f\n"
            "R3D7C visual contract: R3D7B_DEPTH_HUD_FLAT_PLUS_CMDBUF_HEADROOM\n"
            "Clean Shutdown: YES\n"
            "============================================================\n",
            citadel_run_evidence_token,
            r3d7b_depth_proven ? "YES" : "NO",
            citadel_r3d7a_stereo_frames,
            citadel_r3d7a_scene_traversals,
            citadel_r3d7a_right_traversals_skipped,
            citadel_r3d7a_left_parity_passes,
            citadel_r3d7a_left_capture_bypassed,
            citadel_r3d7a_legacy_compose_bypassed,
            citadel_r3d7a_unexpected_right_passes,
            citadel_r3d7a_right_software_frame_activations,
            (unsigned long long)stats.left_triangles_captured,
            (unsigned long long)stats.right_triangles_captured,
            (unsigned long long)citadel_r3d7a_parity_right_native_draw_calls,
            (unsigned long long)citadel_r3d7a_parity_right_native_triangles,
            (unsigned long long)citadel_r3d7b_right_warp_draws,
            (unsigned long long)citadel_r3d7b_right_warp_vertices,
            (unsigned long long)citadel_r3d7b_zero_slider_right_draws,
            (double)citadel_r3d7b_max_slider_seen,
            (double)citadel_r3d7b_min_source_w,
            (double)citadel_r3d7b_max_source_w,
            (double)citadel_r3d7b_min_shift_pixels,
            (double)citadel_r3d7b_max_shift_pixels,
            (double)CITADEL_R3D7B_STEREO_NEAR_CLAMP_W,
            (double)CITADEL_R3D7B_STEREO_CONVERGENCE_W,
            (double)CITADEL_R3D7B_STEREO_MAX_RECIP_SCALE,
            (double)CITADEL_R3D7B_STEREO_MAX_SHIFT_PIXELS,
            (unsigned long long)citadel_gpu_foreground_frames_built,
            (unsigned long long)citadel_gpu_foreground_right_frames_built,
            (unsigned long long)citadel_gpu_foreground_left_draws,
            (unsigned long long)citadel_r3d7a_parity_right_foreground_draws,
            (unsigned long long)citadel_gpu_foreground_build_failures,
            (unsigned long long)citadel_gpu_foreground_right_build_failures,
            (unsigned long long)citadel_gpu_foreground_draw_failures,
            (unsigned long long)stats.capture_overflows,
            (unsigned long long)stats.draw_budget_drops,
            (unsigned long long)citadel_native_texture_upload_failures,
            citadel_gpu_upload_failures,
            citadel_gpu_draw_failures,
            (unsigned int)CITADEL_R3D7C_CMDBUF_SIZE,
            citadel_r3d7c_cmd_buf_samples,
...
static float citadel_native_worldproof_map_source_x(
    const CitadelNativeWorldVertex *vertex,
    ...
    float destination_w)
{
    ...
    return destination_x +
        ((vertex->source_x - (float)source_rect->x) /
         (float)source_rect->w) *
        destination_w;
}

static float citadel_native_worldproof_map_source_y(
    const CitadelNativeWorldVertex *vertex,
    const SDL_Rect *source_rect,
    float destination_y,
    float destination_h)
{
    ...
    return destination_y +
        ((vertex->source_y - (float)source_rect->y) /
         (float)source_rect->h) *
        destination_h;
}

static float citadel_native_worldproof_homogeneous_w(
    const CitadelNativeWorldVertex *vertex)
{
    float value;
    ...
    if (!isfinite(value) || value < 0.001f)
        value = 0.001f;
    if (value > 4096.0f)
        value = 4096.0f;
...
static void citadel_r3d7b_begin_physical_right_warp(void)
{
    ...
    citadel_r3d7b_right_warp_strength =
        slider * CITADEL_R3D7B_STEREO_MAX_RECIP_SCALE;
    citadel_r3d7b_right_warp_active = slider > 0.0001f;

    ++citadel_r3d7b_right_warp_draws;

    if (!citadel_r3d7b_right_warp_active)
        ++citadel_r3d7b_zero_slider_right_draws;

    if (slider > citadel_r3d7b_max_slider_seen)
        citadel_r3d7b_max_slider_seen = slider;
}

static void citadel_r3d7b_end_physical_right_warp(void)
{
    citadel_r3d7b_right_warp_active = false;
    citadel_r3d7b_right_warp_strength = 0.0f;
}

static float citadel_r3d7b_warp_screen_x(
    float screen_x,
    float homogeneous_w)
{
    float depth;
    ...
    depth = homogeneous_w;
    if (!isfinite(depth) || depth < CITADEL_R3D7B_STEREO_NEAR_CLAMP_W)
        depth = CITADEL_R3D7B_STEREO_NEAR_CLAMP_W;

    shift_pixels =
        -citadel_r3d7b_right_warp_strength *
        ((1.0f / depth) -
         (1.0f / CITADEL_R3D7B_STEREO_CONVERGENCE_W));

    if (shift_pixels < -CITADEL_R3D7B_STEREO_MAX_SHIFT_PIXELS)
        shift_pixels = -CITADEL_R3D7B_STEREO_MAX_SHIFT_PIXELS;
    if (shift_pixels > CITADEL_R3D7B_STEREO_MAX_SHIFT_PIXELS)
        shift_pixels = CITADEL_R3D7B_STEREO_MAX_SHIFT_PIXELS;

    ++citadel_r3d7b_right_warp_vertices;

    if (!citadel_r3d7b_warp_range_valid) {
        citadel_r3d7b_warp_range_valid = true;
        citadel_r3d7b_min_source_w = homogeneous_w;
        citadel_r3d7b_max_source_w = homogeneous_w;
        citadel_r3d7b_min_shift_pixels = shift_pixels;
        citadel_r3d7b_max_shift_pixels = shift_pixels;
    } else {
        if (homogeneous_w < citadel_r3d7b_min_source_w)
            citadel_r3d7b_min_source_w = homogeneous_w;
        if (homogeneous_w > citadel_r3d7b_max_source_w)
            citadel_r3d7b_max_source_w = homogeneous_w;
        if (shift_pixels < citadel_r3d7b_min_shift_pixels)
            citadel_r3d7b_min_shift_pixels = shift_pixels;
        if (shift_pixels > citadel_r3d7b_max_shift_pixels)
            citadel_r3d7b_max_shift_pixels = shift_pixels;
    }

    return screen_x + shift_pixels;
}

static void citadel_native_worldproof_send_flat_vertex(
    const CitadelNativeWorldVertex *vertex,
    uint8_t kind,
    const SDL_Rect *source_rect,
    float destination_x,
    ...
    float destination_h)
{
    ...
    case CITADEL_NATIVE_SURFACE_WALL:
        red = 0.06f; green = 0.90f; blue = 0.28f;
        break;
    case CITADEL_NATIVE_SURFACE_FLOOR:
        red = 0.08f; green = 0.24f; blue = 0.95f;
        break;
    case CITADEL_NATIVE_SURFACE_CEILING:
        red = 0.88f; green = 0.08f; blue = 0.82f;
        break;
    default:
        red = 1.0f; green = 0.55f; blue = 0.0f;
        break;
    }

    screen_x = citadel_native_worldproof_map_source_x(
        vertex, source_rect, destination_x, destination_w);
    screen_y = citadel_native_worldproof_map_source_y(
        vertex, source_rect, destination_y, destination_h);
    homogeneous_w =
        citadel_native_worldproof_homogeneous_w(vertex);
    screen_x = citadel_r3d7b_warp_screen_x(
        screen_x,
        homogeneous_w);

    C3D_ImmSendAttrib(
        screen_x * homogeneous_w,
        screen_y * homogeneous_w,
        0.0f,
        homogeneous_w);
    C3D_ImmSendAttrib(red, green, blue, 1.0f);
    ++citadel_native_projected_vertices_sent;
}

static void citadel_native_worldproof_send_textured_vertex(
    const CitadelNativeWorldVertex *vertex,
    const SDL_Rect *source_rect,
    float destination_x,
    float destination_y,
    ...
    float destination_h)
{
    ...
    C3D_ImmSendAttrib(
        ...
        0.0f,
        homogeneous_w);
    C3D_ImmSendAttrib(vertex->u, vertex->v, 0.0f, 0.0f);
    C3D_ImmSendAttrib(
        vertex->light,
        vertex->light,
        vertex->light,
...
static void citadel_native_worldproof_note_screen_mapping(
    const SDL_Rect *source_rect,
    float destination_w,
    float destination_h)
{
    if (source_rect != NULL) {
        citadel_native_projection_last_source_w = source_rect->w;
        citadel_native_projection_last_source_h = source_rect->h;
    }

    citadel_native_projection_last_destination_w =
        (int)(destination_w + 0.5f);
    citadel_native_projection_last_destination_h =
        (int)(destination_h + 0.5f);
}

static void citadel_native_worldproof_prepare_flat_state(void)
{
    C3D_AttrInfo *attribute_info;
    C3D_TexEnv *environment;
    int stage;

    C3D_BindProgram(&citadel_native_worldproof_program);

    attribute_info = C3D_GetAttrInfo();
    AttrInfo_Init(attribute_info);
    AttrInfo_AddLoader(attribute_info, 0, GPU_FLOAT, 4);
    AttrInfo_AddLoader(attribute_info, 1, GPU_FLOAT, 4);
    ...
    C3D_FVUnifMtx4x4(
        GPU_VERTEX_SHADER,
        citadel_native_worldproof_model_view_uniform,
        &citadel_native_worldproof_model_view);
...
static void citadel_native_worldproof_prepare_textured_state(void)
{
    C3D_AttrInfo *attribute_info;
    C3D_TexEnv *environment;
    int stage;

    C3D_BindProgram(&citadel_native_worldtextured_program);

    attribute_info = C3D_GetAttrInfo();
    AttrInfo_Init(attribute_info);
    AttrInfo_AddLoader(attribute_info, 0, GPU_FLOAT, 4);
    AttrInfo_AddLoader(attribute_info, 1, GPU_FLOAT, 2);
    AttrInfo_AddLoader(attribute_info, 2, GPU_FLOAT, 4);
...
static bool citadel_native_worldproof_draw_eye(
    C3D_RenderTarget *target,
    bool right_eye,
    const SDL_Rect *source_rect,
    float destination_x,
    ...
    float destination_h)
{
    const CitadelNativeWorldTriangle *triangles;
    CitadelNativeTextureCacheEntry *
        resolved_entries[CITADEL_NATIVE_WORLD_DRAW_BUDGET];
    size_t captured_count = 0;
    size_t draw_count;
    size_t dropped;
    size_t index;
    size_t textured_count = 0;
    size_t door_textured_count = 0;
    size_t alpha_door_textured_count = 0;
    size_t world_bitmap_textured_count = 0;
    size_t critter_bitmap_textured_count = 0;
    size_t multiview_bitmap_textured_count = 0;
    size_t texbitmap_textured_count = 0;
    size_t flat_count = 0;
    int active_mode = -1;
    C3D_Tex *active_texture = NULL;
    bool run_open = false;
    bool r3d7b_physical_right_warp = false;

    (void)source_rect;

    /* R3D3A: paused/wrapper and stereo frames keep complete software output. */
    if (!citadel_native_world_foreground_present_active())
        return true;

    if (target == NULL || !citadel_native_worldproof_shader_ready)
        return true;

    if (destination_x != 0.0f ||
        destination_y != 0.0f ||
        destination_w < 399.5f ||
        destination_h < 239.5f)
        return true;

    triangles = citadel_native_world_get_triangles(
        right_eye ? 1 : 0,
        &captured_count);

    if (triangles == NULL || captured_count == 0)
        return true;

    draw_count = captured_count;

    if (draw_count > CITADEL_NATIVE_WORLD_DRAW_BUDGET)
        draw_count = CITADEL_NATIVE_WORLD_DRAW_BUDGET;

    dropped = captured_count - draw_count;

    citadel_native_worldproof_note_screen_mapping(
        source_rect,
        destination_w,
        destination_h);
    ...
    for (index = 0; index < draw_count; ++index) {
        ...
        if (citadel_native_worldtextured_shader_ready)
            entry = citadel_native_texture_get(
                &triangles[index].bitmap,
                triangles[index].texture_content_key);

        resolved_entries[index] = entry;

        if (entry != NULL) {
            ++textured_count;
            if (triangles[index].kind == CITADEL_NATIVE_SURFACE_DOOR) {
                ++door_textured_count;
                if (triangles[index].bitmap.flags & BMF_TRANS)
                    ++alpha_door_textured_count;
            } else if (triangles[index].kind ==
                       CITADEL_NATIVE_SURFACE_WORLD_BITMAP) {
                ++world_bitmap_textured_count;

                if (triangles[index].bitmap_category ==
                    CITADEL_NATIVE_BITMAP_CATEGORY_CRITTER) {
                    ++critter_bitmap_textured_count;
                } else if (triangles[index].bitmap_category ==
                           CITADEL_NATIVE_BITMAP_CATEGORY_MULTIVIEW) {
                    ++multiview_bitmap_textured_count;
                }
            } else if (triangles[index].kind ==
                       CITADEL_NATIVE_SURFACE_TEXBITMAP) {
                ++texbitmap_textured_count;
            }
        } else {
            ++flat_count;
        }
    ...
    r3d7b_physical_right_warp =
        target == citadel_gpu_top_right_target &&
        !right_eye &&
        citadel_3ds_stereo_frame_enabled();

    if (r3d7b_physical_right_warp)
        citadel_r3d7b_begin_physical_right_warp();
    ...
    C3D_CullFace(GPU_CULL_NONE);
    /*
     * R3D4D submits exact-projected terrain at equal depth. GEQUAL lets later
     * polygons win, matching the software renderer's painter ordering.
     */
    C3D_DepthTest(true, GPU_GEQUAL, GPU_WRITE_ALL);

    for (index = 0; index < draw_count; ++index) {
        const CitadelNativeWorldTriangle *triangle = &triangles[index];
        CitadelNativeTextureCacheEntry *entry = resolved_entries[index];
        C3D_Tex *texture = entry != NULL ? &entry->texture : NULL;
        const int alpha_texture =
            entry != NULL && entry->texture_format == GPU_RGBA5551;
        const int requested_mode = alpha_texture ? 2 : 1;
        int vertex_index;
        ...
        if (texture != NULL) {
            if (active_mode != requested_mode) {
                ...
                citadel_native_worldproof_prepare_textured_state();
                C3D_AlphaTest(
                    alpha_texture,
                    alpha_texture ? GPU_GREATER : GPU_ALWAYS,
                    0);
            ...
            if (active_texture != texture) {
                ...
                C3D_ImmDrawBegin(GPU_TRIANGLES);
                run_open = true;
                ++citadel_native_texture_draw_runs;
            }

            for (vertex_index = 0; vertex_index < 3; ++vertex_index) {
                citadel_native_worldproof_send_textured_vertex(
            ...
            if (active_mode != 0) {
                ...
                citadel_native_worldproof_prepare_flat_state();
                active_mode = 0;
                active_texture = NULL;
                C3D_ImmDrawBegin(GPU_TRIANGLES);
                run_open = true;
            ...
            for (vertex_index = 0; vertex_index < 3; ++vertex_index) {
                citadel_native_worldproof_send_flat_vertex(
                    &triangle->vertices[vertex_index],
                    triangle->kind,
                    source_rect,
                    destination_x,
    ...
    if (r3d7b_physical_right_warp)
        citadel_r3d7b_end_physical_right_warp();

    citadel_native_textured_triangles_drawn += textured_count;
    citadel_native_door_textured_triangles_drawn += door_textured_count;
    citadel_native_alpha_door_textured_triangles_drawn +=
        alpha_door_textured_count;
    citadel_native_world_bitmap_textured_triangles_drawn +=
        world_bitmap_textured_count;
    citadel_native_critter_bitmap_textured_triangles_drawn +=
        critter_bitmap_textured_count;
    citadel_native_multiview_bitmap_textured_triangles_drawn +=
        multiview_bitmap_textured_count;
    citadel_native_texbitmap_textured_triangles_drawn +=
        texbitmap_textured_count;
    citadel_native_flat_fallback_triangles_drawn += flat_count;

    if (target == citadel_gpu_top_right_target &&
        !right_eye &&
        citadel_3ds_stereo_frame_enabled()) {
        ++citadel_r3d7a_parity_right_native_draw_calls;
        citadel_r3d7a_parity_right_native_triangles +=
            (uint64_t)draw_count;
    }

    citadel_native_world_note_draw(
        right_eye ? 1 : 0,
        draw_count,
        dropped);

    {
        bool *first_draw_logged = right_eye
            ? &citadel_native_worldproof_first_right_draw_logged
            : &citadel_native_worldproof_first_left_draw_logged;

        if (!*first_draw_logged) {
            v5_log("GPU C3D R3D5B2D FIRST DRAW eye=%s captured=%u "
                   "textured=%u doors=%u alpha_doors=%u world_bitmaps=%u critters=%u multiview=%u texbitmaps=%u flat_fallback=%u dropped=%u",
                   right_eye ? "RIGHT" : "LEFT",
                   (unsigned int)captured_count,
                   (unsigned int)textured_count,
                   (unsigned int)door_textured_count,
                   (unsigned int)alpha_door_textured_count,
                   (unsigned int)world_bitmap_textured_count,
                   (unsigned int)critter_bitmap_textured_count,
                   (unsigned int)multiview_bitmap_textured_count,
                   (unsigned int)texbitmap_textured_count,
...
static void citadel_gpu_shutdown(void)
{
    ...
    citadel_native_worldproof_write_summary();
    citadel_native_worldproof_shutdown();
    ...
    citadel_gpu_foreground_shutdown();
...
static bool citadel_v16_load_splash(void)
{
    ...
    for (i = 0; i < sizeof(paths) / sizeof(paths[0]); ++i) {
        ...
        if (citadel_v16_splash_sheet == NULL) {
            v5_log("GPU V16.1 SPLASH LOAD path=%s result=FAIL",
                   paths[i]);
            continue;
        }

        if (C2D_SpriteSheetCount(citadel_v16_splash_sheet) < 1) {
            v5_log("GPU V16.1 SPLASH LOAD path=%s result=FAIL reason=no-images",
                   paths[i]);
            C2D_SpriteSheetFree(citadel_v16_splash_sheet);
            citadel_v16_splash_sheet = NULL;
            continue;
        ...
        if (citadel_v16_splash_image.tex == NULL ||
            ...
            v5_log("GPU V16.1 SPLASH LOAD path=%s result=FAIL reason=invalid-image",
                   paths[i]);
            C2D_SpriteSheetFree(citadel_v16_splash_sheet);
            citadel_v16_splash_sheet = NULL;
            continue;
        ...
        v5_log("GPU V16.1 SPLASH LOAD path=%s result=SUCCESS "
    ...
    v5_log("GPU V16.1 SPLASH unavailable; continuing directly to game");
    return false;
}
...
static bool citadel_gpu_initialize(void)
{
    ...
    memset(&citadel_gpu_right_texture, 0, sizeof(citadel_gpu_right_texture));
     ...
     */
    if (!C3D_Init(CITADEL_R3D7C_CMDBUF_SIZE)) {
        v5_log("GPU INIT FAIL stage=C3D_Init cmdbuf=%u",
               (unsigned int)CITADEL_R3D7C_CMDBUF_SIZE);
        return false;
    }
    ...
    citadel_native_world_reset_all();
    citadel_native_worldproof_initialize();
    citadel_native_worldproof_write_start();

    v5_log("GPU C3D R3D6B1 NATIVE PRESENTATION CUTOVER1 ACTIVE "
           "mode=NATIVE_TERRAIN_PLUS_PAINTER_ORDER_KEY_ERASE_FOREGROUND "
           "flat_shader=%u textured_shader=%u "
           "software_terrain=TEXTURE_MAPPING_SUPPRESSED "
           "terrain_occlusion=FLAT_KEY_ZERO_ERASE projection=SOFTWARE_PROJECTED_SX_SY "
           "opaque_doors=NATIVE_STABLE_SNAPSHOT_KEY_ERASE "
           "transparent_doors=NATIVE_RGBA5551_ALPHA_MASK_ERASE "
           "world_bitmap=STANDARD_FAUBJ_BITMAP_RGBA5551_ALPHA_MASK_ERASE "
           "critter_bitmap=NORMAL_FAUBJ_CRIT_RGBA5551_ALPHA_MASK_ERASE "
           "multiview_bitmap=FAUBJ_MULTIVIEW_RGBA5551_ALPHA_MASK_ERASE "
           "legacy_blend_flag=SCALE_FILTER_HINT_ACCEPTED "
           "per_pixel_reference=DISABLED "
           "bitmap_identity=VALUE_SNAPSHOT "
           "cache_capacity=%u capture_max=%u draw_budget=%u",
           citadel_native_worldproof_shader_ready ? 1U : 0U,
           citadel_native_worldtextured_shader_ready ? 1U : 0U,
           (unsigned int)CITADEL_NATIVE_TEXTURE_CACHE_CAPACITY,
           (unsigned int)CITADEL_NATIVE_WORLD_MAX_TRIANGLES,
           (unsigned int)CITADEL_NATIVE_WORLD_DRAW_BUDGET);
    ...
    v5_log("GPU 3D S3 mode=TRUE_WORLD_AUTO_FLAT_UI "
           "fallback=IDENTICAL_EYES max_convergence_px=5 "
           "curve=CENTERED slider=%0.3f left=%p right=%p bottom=%p",
    ...
    if (!C3D_TexInit(&citadel_gpu_right_texture,
                     CITADEL_GPU_TEXTURE_WIDTH,
                     CITADEL_GPU_TEXTURE_HEIGHT,
                     GPU_RGB565)) {
        v5_log("GPU 3D S2 INIT FAIL stage=right-C3D_TexInit size=%dx%d",
               CITADEL_GPU_TEXTURE_WIDTH, CITADEL_GPU_TEXTURE_HEIGHT);
        citadel_gpu_shutdown();
        citadel_gpu_shutdown_complete = false;
        return false;
    }
    citadel_gpu_right_texture_initialized = true;
    ...
    /* Non-fatal: allocation failure retains the fully proven R3C path. */
    (void)citadel_gpu_foreground_initialize();
    ...
    v5_log("GPU V16.1 LAUNCH PIPELINE ready texture=%dx%d content=%dx%d "
    ...
    v5_log("GPU INIT SUCCESS backend=Citro2D texture=%dx%d format=RGB565 "
           "left=%p right=%p bottom=%p left_tex=%p right_tex=%p staging=%p",
           CITADEL_GPU_TEXTURE_WIDTH,
           CITADEL_GPU_TEXTURE_HEIGHT,
           (void *)citadel_gpu_top_target,
           (void *)citadel_gpu_top_right_target,
           ...
           citadel_gpu_right_texture.data,
...
static unsigned int citadel_gpu_refresh_palette565(void)
{
    ...
    if (citadel_gpu_palette_refreshes == 1 ||
        ...
        v5_log("GPU V16.1 PALETTE SYNC refresh=%lu mismatches=%u "
 ...
 */
static uint32_t citadel_gpu_build_linear_indexed(const u8 *source_pixels,
                                                   int source_width,
                                                   int source_height,
                                                   int source_pitch)
{
    ...
    if (source_pixels == NULL || source_width <= 0 || source_height <= 0 ||
        source_pitch < source_width) return 0;
    memset(citadel_gpu_staging, 0, CITADEL_GPU_STAGING_BYTES);
    for (destination_y = 0; destination_y < CITADEL_GPU_CONTENT_HEIGHT;
         ++destination_y) {
        int source_y = (destination_y * source_height) /
                       CITADEL_GPU_CONTENT_HEIGHT;
        const u8 *source_row;
        u16 *destination_row;
        int destination_x;
        if (source_y >= source_height) source_y = source_height - 1;
        source_row = source_pixels + (size_t)source_y * source_pitch;
        destination_row = citadel_gpu_staging +
            (size_t)destination_y * CITADEL_GPU_TEXTURE_WIDTH;
        for (destination_x = 0; destination_x < CITADEL_GPU_CONTENT_WIDTH;
             ++destination_x) {
            int source_x = (destination_x * source_width) /
                           CITADEL_GPU_CONTENT_WIDTH;
            u16 converted;
            if (source_x >= source_width) source_x = source_width - 1;
            converted = citadel_gpu_palette565[source_row[source_x]];
...
{
    return citadel_gpu_build_linear_indexed((const u8 *)surface->pixels,
 ...
 */
static void citadel_gpu_swizzle_staging_to_texture(C3D_Tex *texture)
{
    u16 *texture_pixels;
    int y;
    ...
    for (y = 0; y < CITADEL_GPU_CONTENT_HEIGHT; ++y) {
        const u16 *source_row = citadel_gpu_staging +
            (size_t)y * CITADEL_GPU_TEXTURE_WIDTH;
        int x;
        for (x = 0; x < CITADEL_GPU_CONTENT_WIDTH; ++x)
            texture_pixels[citadel_gpu_tiled_offset((unsigned int)x,
 ...
 * PROJECT CITADEL C3D R3D1 KEYED FOREGROUND COMPOSITE1
 * ------------------------------------------------------------------------- */
static inline u16 citadel_gpu_foreground_color_to_rgba5551(
    SDL_Color color,
    bool opaque)
{
    const u16 r5 = (u16)(color.r >> 3);
...
static void citadel_gpu_foreground_shutdown(void)
{
    ...
    for (set_index = 0; set_index < 2; ++set_index) {
        if (citadel_gpu_foreground_texture_initialized[set_index]) {
            C3D_TexDelete(&citadel_gpu_foreground_texture_sets[set_index]);
            citadel_gpu_foreground_texture_initialized[set_index] = false;
        }

        if (citadel_gpu_foreground_right_texture_initialized[set_index]) {
            C3D_TexDelete(
                &citadel_gpu_foreground_right_texture_sets[set_index]);
            citadel_gpu_foreground_right_texture_initialized[set_index] =
                false;
        }
    }

    citadel_gpu_foreground_enabled = false;
    citadel_gpu_foreground_ready_this_frame = false;
    citadel_gpu_foreground_right_ready_this_frame = false;
    citadel_gpu_foreground_texture_live =
        &citadel_gpu_foreground_texture_sets[0];
    citadel_gpu_foreground_right_texture_live =
        &citadel_gpu_foreground_right_texture_sets[0];
}
...
static bool citadel_gpu_foreground_initialize(void)
{
    ...
    memset(citadel_gpu_foreground_texture_sets,
           0,
           sizeof(citadel_gpu_foreground_texture_sets));

    for (set_index = 0; set_index < 2; ++set_index) {
        C3D_Tex *texture =
            &citadel_gpu_foreground_texture_sets[set_index];
        ...
        if (!C3D_TexInit(
                ...
                CITADEL_GPU_CROPPED_TEXTURE_HEIGHT,
                GPU_RGBA5551)) {
            v5_log("GPU C3D R3D1 FOREGROUND INIT FALLBACK set=%d",
                   set_index);
            citadel_gpu_foreground_shutdown();
            return false;
        }

        citadel_gpu_foreground_texture_initialized[set_index] = true;
        C3D_TexSetFilter(texture, GPU_NEAREST, GPU_NEAREST);
        C3D_TexSetWrap(texture, GPU_CLAMP_TO_EDGE, GPU_CLAMP_TO_EDGE);
        memset(texture->data, 0, CITADEL_GPU_FOREGROUND_TEXTURE_BYTES);
        GSPGPU_FlushDataCache(
            texture->data,
            CITADEL_GPU_FOREGROUND_TEXTURE_BYTES);
        ...
        if (!C3D_TexInit(
                ...
                CITADEL_GPU_CROPPED_TEXTURE_HEIGHT,
                GPU_RGBA5551)) {
            v5_log("GPU C3D R3D6B1 RIGHT FOREGROUND INIT FALLBACK "
                   "set=%d",
                   set_index);
            citadel_gpu_foreground_shutdown();
            return false;
        ...
        citadel_gpu_foreground_right_texture_initialized[set_index] = true;
        C3D_TexSetFilter(texture, GPU_NEAREST, GPU_NEAREST);
        C3D_TexSetWrap(texture, GPU_CLAMP_TO_EDGE, GPU_CLAMP_TO_EDGE);
        memset(texture->data, 0, CITADEL_GPU_FOREGROUND_TEXTURE_BYTES);
        GSPGPU_FlushDataCache(
    ...
    citadel_gpu_foreground_enabled = true;
    citadel_gpu_foreground_texture_live =
        &citadel_gpu_foreground_texture_sets[0];
    citadel_gpu_foreground_right_texture_live =
        &citadel_gpu_foreground_right_texture_sets[0];

    v5_log("GPU C3D R3D6B1 FOREGROUND INIT SUCCESS "
           "sets=2 eyes=2 format=RGBA5551 size=%dx%d key_index=%u "
           "bytes_each=%u",
           CITADEL_GPU_CROPPED_TEXTURE_WIDTH,
           CITADEL_GPU_CROPPED_TEXTURE_HEIGHT,
           (unsigned int)CITADEL_GPU_FOREGROUND_KEY_INDEX,
           (unsigned int)CITADEL_GPU_FOREGROUND_TEXTURE_BYTES);
    return true;
}

static bool citadel_gpu_foreground_build(
    SDL_Surface *surface,
    const citadel_gpu_cropped_map *map)
{
    const u8 *source_pixels;
    u16 *texture_pixels;
    uint64_t opaque_texels = 0;
    uint64_t keyed_texels = 0;
    int tile_y;

    citadel_gpu_foreground_ready_this_frame = false;

    if (!citadel_gpu_foreground_enabled ||
        !citadel_native_world_foreground_present_active())
        return true;

    if (surface == NULL || surface->pixels == NULL ||
        map == NULL || !map->valid ||
        map->destination_w != CITADEL_GPU_CROPPED_TOP_SPLIT_WIDTH ||
        map->destination_h != CITADEL_GPU_CROPPED_TOP_HEIGHT ||
        citadel_gpu_foreground_texture_live == NULL ||
        citadel_gpu_foreground_texture_live->data == NULL)
        return false;

    /*
     * R3D4A: terrain polygons erase their coverage to palette index zero in
     * the ordinary foreground canvas, in original software painter order.
     * Therefore no second terrain canvas and no per-pixel reference
     * comparison are required here.
     */
    source_pixels = (const u8 *)surface->pixels;
    texture_pixels = (u16 *)citadel_gpu_foreground_texture_live->data;
    memset(texture_pixels, 0, CITADEL_GPU_FOREGROUND_TEXTURE_BYTES);

    for (tile_y = 0;
         tile_y < CITADEL_GPU_CROPPED_TOP_HEIGHT / 8;
         ++tile_y) {
        int tile_x;

        for (tile_x = 0;
             tile_x < CITADEL_GPU_CROPPED_TOP_SPLIT_WIDTH / 8;
             ++tile_x) {
            const size_t tile_base =
                (((size_t)tile_y *
                  (size_t)(CITADEL_GPU_CROPPED_TEXTURE_WIDTH / 8)) +
            ...
            for (morton = 0; morton < 64u; ++morton) {
                const unsigned int local_x =
                    (morton & 1u) |
                    ((morton >> 1) & 2u) |
                    ((morton >> 2) & 4u);
                ...
                const int output_y = tile_y * 8 + (int)local_y;
                const int source_x =
                    map->source_x_map[output_x];
                const size_t source_offset =
                    map->source_y_offset[output_y] +
                    (size_t)source_x;
                const unsigned int palette_index =
                    source_pixels[source_offset];

                if (palette_index ==
                    (unsigned int)CITADEL_GPU_FOREGROUND_KEY_INDEX) {
                    texture_pixels[tile_base + morton] = 0;
                    ++keyed_texels;
                } else {
                    const SDL_Color color = sdlPalette != NULL
                        ? sdlPalette->colors[palette_index]
                    ...
                    texture_pixels[tile_base + morton] =
                        citadel_gpu_foreground_color_to_rgba5551(
                            color,
                            true);
                    ++opaque_texels;
                }
    ...
    GSPGPU_FlushDataCache(
        citadel_gpu_foreground_texture_live->data,
        CITADEL_GPU_FOREGROUND_TEXTURE_BYTES);

    citadel_gpu_foreground_ready_this_frame = true;
    ++citadel_gpu_foreground_frames_built;
    citadel_gpu_foreground_opaque_texels += opaque_texels;
    citadel_gpu_foreground_keyed_texels += keyed_texels;
    return true;
}

/*
 * R3D6B2: fuse right-eye software residual + single shared overlay pass into a
 * separate keyed foreground without constructing a full 640x480 composed right frame.
 * Native-owned world pixels are palette index zero; unsupported software objects
 * stay eye-specific while shared HUD/overlay changes are copied from the left frame.
 */
static bool citadel_gpu_foreground_build_right(
    const u8 *source_pixels,
    int source_width,
    int source_height,
    int source_pitch,
    const u8 *left_final_pixels,
    const u8 *left_baseline_pixels,
    bool direct_foreground,
    ...
    const citadel_gpu_cropped_map *map)
{
    ...
    if (source_pixels == NULL ||
        ...
        (direct_foreground &&
         (left_final_pixels == NULL || left_baseline_pixels == NULL)) ||
        map == NULL || !map->valid ||
        map->destination_w != CITADEL_GPU_CROPPED_TOP_SPLIT_WIDTH ||
        map->destination_h != CITADEL_GPU_CROPPED_TOP_HEIGHT ||
        citadel_gpu_foreground_right_texture_live == NULL ||
        citadel_gpu_foreground_right_texture_live->data == NULL)
        return false;

    texture_pixels =
        (u16 *)citadel_gpu_foreground_right_texture_live->data;
    memset(texture_pixels, 0, CITADEL_GPU_FOREGROUND_TEXTURE_BYTES);
        ...
        for (tile_x = 0;
            ...
            for (morton = 0; morton < 64u; ++morton) {
                ...
                if (direct_foreground &&
                    left_final_pixels[source_offset] !=
                        left_baseline_pixels[source_offset]) {
                    /*
                     * The single real overlay/HUD pass runs only on the left
                     * eye. A changed pixel is therefore a shared flat overlay
                     * and is fused into the right keyed foreground here.
                     */
                    palette_index = left_final_pixels[source_offset];
                    ++citadel_r3d6b2_overlay_pixels_fused;
                } else if (direct_foreground) {
                    const int shifted_source_x =
                        source_x - convergence_shift;

                    if (shifted_source_x < 0 ||
                        shifted_source_x >= source_width) {
                        palette_index =
                            CITADEL_GPU_FOREGROUND_KEY_INDEX;
                    } else {
                        palette_index = source_pixels[
                            map->source_y_offset[output_y] +
                            (size_t)shifted_source_x];
                        ++citadel_r3d6b2_right_residual_pixels_sampled;
                    }
                } else {
                    palette_index = source_pixels[source_offset];
                }
    ...
    GSPGPU_FlushDataCache(
        citadel_gpu_foreground_right_texture_live->data,
        CITADEL_GPU_FOREGROUND_TEXTURE_BYTES);

    citadel_gpu_foreground_right_ready_this_frame = true;
    ++citadel_gpu_foreground_right_frames_built;
    citadel_gpu_foreground_right_opaque_texels += opaque_texels;
    citadel_gpu_foreground_right_keyed_texels += keyed_texels;
    return true;
}

static bool citadel_gpu_foreground_draw(
    C3D_RenderTarget *target,
    bool right_eye)
{
    C3D_Tex *foreground_texture;
    bool draw_ok;

    if (right_eye && citadel_gpu_right_frame_active) {
        if (!citadel_gpu_foreground_right_ready_this_frame)
            return true;
        foreground_texture = citadel_gpu_foreground_right_texture_live;
    } else {
        if (!citadel_gpu_foreground_ready_this_frame)
            return true;
        foreground_texture = citadel_gpu_foreground_texture_live;
    }

    if (!citadel_gpu_directquad_begin_scene(target, false)) {
        ++citadel_gpu_foreground_draw_failures;
        return false;
    }

    /* RGBA5551 is binary alpha: discard key texels, replace all others. */
    C3D_AlphaTest(true, GPU_GREATER, 0);
    draw_ok = citadel_gpu_directquad_draw_texel_region(
        foreground_texture,
        CITADEL_GPU_CROPPED_TEXTURE_WIDTH,
        CITADEL_GPU_CROPPED_TEXTURE_HEIGHT,
        0,
    ...
    if (right_eye) {
        ++citadel_gpu_foreground_right_draws;
        if (!citadel_gpu_right_frame_active &&
            citadel_3ds_stereo_frame_enabled())
            ++citadel_r3d7a_parity_right_foreground_draws;
    } else {
        ++citadel_gpu_foreground_left_draws;
    }
    ...
    uint32_t *linear_hash)
{
    ...
    needs_validation = !cutover_candidate &&
        (split_layout
            ? !citadel_gpu_croppedtiled_split_left_validated
            : !citadel_gpu_croppedtiled_legacy_left_validated);
    ...
    /*
     * R3D6B1: build the keyed foreground before omitting the software top
     * transport. Any foreground failure restores the old RGB565 top path in
     * this same frame.
     */
    if (split_layout &&
        citadel_native_world_foreground_present_active()) {
        if (!citadel_gpu_foreground_build(
                surface,
                &citadel_gpu_cropped_top_split_map)) {
            ++citadel_gpu_foreground_build_failures;
            citadel_gpu_foreground_ready_this_frame = false;
            citadel_native_world_occlusion_reference_invalidate();
        }
    }
    ...
    skip_top_transport =
        cutover_candidate &&
        citadel_native_world_foreground_present_active() &&
        citadel_gpu_foreground_ready_this_frame;

    if (skip_top_transport) {
        ++citadel_r3d6b1_top_left_uploads_skipped;
    ...
    bool cutover_candidate)
{
    ...
    if (split_layout &&
        citadel_native_world_foreground_present_active()) {
        if (!citadel_gpu_foreground_build_right(
                source_pixels,
                source_width,
                source_height,
                source_pitch,
                left_final_pixels,
                left_baseline_pixels,
                direct_foreground,
                convergence_shift,
                &citadel_gpu_cropped_top_split_map)) {
            int fallback_width = 0;
            int fallback_height = 0;
            int fallback_pitch = 0;
            float fallback_slider = 0.0f;
            const u8 *fallback_pixels = NULL;

            ++citadel_gpu_foreground_right_build_failures;
            citadel_gpu_foreground_right_ready_this_frame = false;

            /*
             * Direct fusion is fail-safe. The right and partial-left software
             * residuals are still alive, so recover the legacy composed top
             * frame for this same presentation if the direct foreground path
             * cannot be built.
             */
            if (direct_foreground &&
                citadel_3ds_stereo_force_legacy_compositor()) {
                fallback_pixels = citadel_3ds_stereo_get_right_frame(
                    &fallback_width,
                    &fallback_height,
                    &fallback_pitch,
                    &fallback_slider);

                if (fallback_pixels != NULL &&
                    fallback_width == source_width &&
                    fallback_height == source_height &&
                    fallback_pitch >= source_width) {
                    source_pixels = fallback_pixels;
                    direct_foreground = false;
                }
            }
        }
    }
    ...
    skip_top_transport =
        cutover_candidate &&
        citadel_native_world_foreground_present_active() &&
        citadel_gpu_foreground_right_ready_this_frame;

    if (skip_top_transport) {
        ++citadel_r3d6b1_top_right_uploads_skipped;
        needs_validation = false;
    } else {
        if (!citadel_gpu_croppedtiled_write_region(
                                         ...
                                         C3D_Tex *texture)
{
    ...
    if (texture == NULL) {
        v5_log("GPU V16.1 TEX %s NULL",
               label != NULL ? label : "(null)");
        return;
    }

    v5_log("GPU V16.1 TEX %s data=%p phys=0x%08lX "
                                                     ...
                                                     uint32_t linear_hash)
{
    ...
    v5_log("GPU V16.1 LOAD VERIFY rgb565_memcmp=%d "
    ...
    v5_log("GPU V16.1 PHASE begin=%s duration_ms=6000 "
...
static bool citadel_gpu_v15h_load_sheet(void)
{
    ...
    for (i = 0; i < sizeof(paths) / sizeof(paths[0]); ++i) {
        ...
        if (citadel_gpu_v15h_sheet != NULL) {
            v5_log("GPU V16.1 TEX3DS LOAD path=%s result=SUCCESS count=%lu",
                   paths[i],
        ...
        v5_log("GPU V16.1 TEX3DS LOAD path=%s result=FAIL",
               paths[i]);
    }
...
static bool citadel_gpu_v15h_initialize_control(void)
{
    ...
    if (C2D_SpriteSheetCount(citadel_gpu_v15h_sheet) < 1) {
        v5_log("GPU V16.1 TEX3DS LOAD FAIL reason=no-images");
        return false;
    }
    ...
    if (citadel_gpu_v15h_official_image.tex == NULL ||
        ...
        v5_log("GPU V16.1 TEX3DS LOAD FAIL reason=invalid-image");
        return false;
    }
    ...
    v5_log("GPU V16.1 SUBTEX official width=%u height=%u "
    ...
    if (citadel_gpu_v15h_official_image.tex->width !=
        ...
        v5_log("GPU V16.1 TEX3DS LOAD FAIL "
    ...
    if (!C3D_TexInit(&citadel_gpu_v15h_manual_texture,
                     ...
                     CITADEL_V15H_CONTROL_SIZE,
                     GPU_RGB565)) {
        v5_log("GPU V16.1 MANUAL INIT FAIL stage=C3D_TexInit");
        return false;
    }
    ...
    if (citadel_gpu_v15h_candidate == NULL) {
        v5_log("GPU V16.1 MANUAL INIT FAIL stage=linearAlloc bytes=%u",
    ...
    for (variant = 0; variant <= 5; ++variant) {
        ...
        v5_log("GPU V16.1 GOLDEN COMPARE variant=%d "
    ...
    v5_log("GPU V16.1 CONTROL READY "
...
static bool citadel_gpu_upload_surface(SDL_Surface *surface)
{
    ...
    const unsigned char *left_baseline_pixels = NULL;
    int right_width = 0, right_height = 0, right_pitch = 0;
    int left_baseline_width = 0;
    int left_baseline_height = 0;
    int left_baseline_pitch = 0;
    int direct_convergence_shift = 0;
    float right_slider = 0.0f;
    bool direct_foreground = false;
    u64 profile_tick = 0;
    bool cropped_split_layout = false;
    bool cutover_candidate = false;
    ...
    if (citadel_gpu_v15h_phase ==
        ...
        v5_log("GPU V16.1 PHASE complete; "
    ...
    cutover_candidate =
        citadel_r3d6b1_cutover_candidate(cropped_split_layout);
    ...
    if (citadel_gpu_right_texture_initialized &&
        ...
        direct_foreground =
            citadel_3ds_stereo_direct_foreground_active() != 0;

        if (direct_foreground) {
            left_baseline_pixels =
                citadel_3ds_stereo_get_left_world_baseline(
                    &left_baseline_width,
                    &left_baseline_height,
                    &left_baseline_pitch);
            direct_convergence_shift =
                citadel_3ds_stereo_get_convergence_shift();

            if (left_baseline_pixels == NULL ||
                left_baseline_width != surface->w ||
                left_baseline_height != surface->h ||
                left_baseline_pitch != surface->pitch ||
                right_pitch != surface->pitch) {
                /*
                 * A direct frame without a matching left pre-overlay baseline
                 * cannot safely distinguish shared HUD from eye-specific
                 * residual pixels. Recover the legacy compositor now.
                 */
                if (citadel_3ds_stereo_force_legacy_compositor()) {
                    right_pixels = citadel_3ds_stereo_get_right_frame(
                        &right_width,
                        &right_height,
                        &right_pitch,
                ...
                left_baseline_pixels = NULL;
                direct_foreground = false;
                direct_convergence_shift = 0;
            }
        }
    ...
    if (right_pixels != NULL && right_width == surface->w &&
        ...
        if (!citadel_gpu_first_right_upload_logged) {
            v5_log("GPU 3D S3 FIRST TRUE FRAME src=%dx%d pitch=%d "
                   "slider=%0.3f method=DUAL_CAMERA_AUTO_OVERLAY_MASK",
                   right_width, right_height, right_pitch,
                   (double)right_slider);
            citadel_gpu_first_right_upload_logged = true;
        }
    }
    ...
    if (!citadel_gpu_v15h_capture_done &&
        ...
        if (citadel_gpu_v15h_asset_ready) {
            ...
            citadel_gpu_v15h_phase =
                CITADEL_V15H_PHASE_COMPLETE;
            v5_log("GPU V16.1 PHASE SKIPPED reason=asset-unavailable");
        }
    }
...
static bool citadel_gpu_draw_region_from_texture(C3D_Tex *texture,
                                                 int source_x,
                                                 int source_y,
                                                 int source_w,
                                                 int source_h,
...
static bool citadel_gpu_draw_region(int source_x, int source_y,
                                    ...
                                    float depth)
{
    return citadel_gpu_draw_region_from_texture(&citadel_gpu_texture,
        source_x, source_y, source_w, source_h, destination_x, destination_y,
        destination_w, destination_h, depth);
}
...
static bool citadel_gpu_present(SDL_Surface *surface, bool magenta)
{
    ...
    bool right_ok = true;
    bool bottom_ok = true;
    bool left_native_cutover = false;
    bool right_native_cutover = false;
    SDL_Rect top_source;
    u64 profile_tick = 0;
    ...
    /*
     * Decide the cutover per eye after upload. If a foreground build failed,
     * this frame automatically retains the proven software top base.
     */
    left_native_cutover =
        citadel_r3d6b1_cutover_candidate(split_layout) &&
        citadel_gpu_foreground_ready_this_frame;
    right_native_cutover =
        citadel_r3d6b1_cutover_candidate(split_layout) &&
        (citadel_gpu_right_frame_active
            ? citadel_gpu_foreground_right_ready_this_frame
            : citadel_gpu_foreground_ready_this_frame);

    if (left_native_cutover)
        ++citadel_r3d6b1_cutover_frames;

    if (split_layout &&
        !citadel_native_world_foreground_present_active())
        ++citadel_r3d6b1_exceptional_software_top_frames;
    ...
    if (citadel_gpu_croppedtiled_enabled &&
        ...
        if (left_native_cutover) {
            ++citadel_r3d6b1_top_left_draws_skipped;
        } else {
            ...
            if (split_layout &&
                citadel_native_world_foreground_present_active())
                ++citadel_r3d6b1_takeover_software_top_fallback_draws;
        }

        top_ok = top_ok && citadel_native_worldproof_draw_eye(
            citadel_gpu_top_target,
            false,
            &top_source,
            split_layout ? 0.0f : (float)CITADEL_3DS_GAME_X,
        ...
        if (split_layout)
            top_ok = top_ok && citadel_gpu_foreground_draw(
                citadel_gpu_top_target, false);

        right_ok = citadel_gpu_directquad_begin_scene(
            citadel_gpu_top_right_target,
            false);
        if (right_native_cutover) {
            ++citadel_r3d6b1_top_right_draws_skipped;
        } else {
        ...
        right_ok = right_ok && citadel_native_worldproof_draw_eye(
            citadel_gpu_top_right_target,
            citadel_gpu_right_frame_active,
            &top_source,
            split_layout ? 0.0f : (float)CITADEL_3DS_GAME_X,
        ...
        if (split_layout)
            right_ok = right_ok && citadel_gpu_foreground_draw(
                citadel_gpu_top_right_target, true);
        ...
        /*
         * Safety wall: without the cropped foreground/bottom path, preserve
         * complete software top presentation rather than forcing cutover.
         */
        if (split_layout &&
            citadel_native_world_foreground_present_active())
            citadel_r3d6b1_takeover_software_top_fallback_draws += 2;
    ...
    profile_tick = svcGetSystemTick();
    C3D_FrameEnd(0);
    citadel_native_world_finish_present();
    ...
    if (citadel_gpu_presented_frames == 1) {
        v5_log("GPU 3D R3D7B FIRST DEPTH PRESENT slider=%0.3f split=%d "
               "true_world=%d cutover_left=%d cutover_right=%d "
               "left_ok=%d right_ok=%d bottom_ok=%d surface=%dx%d "
               "warp_scale=%0.3f convergence_w=%0.3f",
               (double)(citadel_gpu_right_frame_active
                   ? citadel_gpu_right_frame_slider : osGet3DSliderState()),
               split_layout ? 1 : 0,
               citadel_gpu_right_frame_active ? 1 : 0,
               left_native_cutover ? 1 : 0,
               right_native_cutover ? 1 : 0,
               top_ok ? 1 : 0,
               right_ok ? 1 : 0,
               bottom_ok ? 1 : 0,
...
static void v5_log(const char *fmt, ...)
{
    FILE *file = fopen("GPU_C2D_V16_1.log", "a");
    va_list args;
...
#if defined(__3DS__) || defined(_3DS)
    /*
     * Both Homebrew Launcher and installed CIA builds use the same external
     * data directory. This also makes prefs, keybinds, saves, logs, music,
     * and the splash independent of the launcher's initial working directory.
     */
    citadel_3ds_working_directory_result =
...
#if defined(__3DS__) || defined(_3DS)
    /*
     * init_all() has opened the shared effects/speech/music SDL device.
     * Register only after that device exists.
     */
    citadel_3ds_audio_register_apt_hook();
#endif
...
#if defined(__3DS__) || defined(_3DS)
    v5_log("APP EXIT mainloop-return system_close=%d cwd_result=%d",
           citadel_3ds_system_close_requested ? 1 : 0,
           citadel_3ds_working_directory_result);
#endif
...
#if defined(__3DS__) || defined(_3DS)
    /*
     * Complete every subsystem in a known order before returning to HOME.
     * The system-close path ends with svcExitProcess() after cleanup so the
     * HOME Menu cannot be left waiting on a stalled final frame.
     */
    citadel_3ds_audio_unregister_apt_hook();
    citadel_gpu_shutdown();
    citadel_sdl_shutdown_once();

    /*
     * Do not call svcExitProcess() here. Returning from main lets libctru's
     * normal APT teardown finish PrepareToCloseApplication/CloseApplication.
     */
#endif
...
void InitSDL() {
    ...
    v5_log("PROJECT CITADEL C3D R3D7C CMDBUF512 SHIPRC1 ACTIVE | "
           "run=%s build=%s %s profile_log=%s",
           citadel_run_evidence_token,
           __DATE__,
           __TIME__,
           CITADEL_C3D_PROFILE_LOG_PATH);
...
#if defined(__3DS__) || defined(_3DS)
    atexit(citadel_sdl_shutdown_once);
#else
    atexit(SDL_Quit);
#endif
...
#if defined(__3DS__) || defined(_3DS)
    citadel_gpu_update_palette565();
    citadel_native_texture_note_palette_changed();
#endif
...
void SDLDraw() {
...
#if defined(__3DS__) || defined(_3DS)
    /* Reopen only after libctru has returned from HOME and woken DSP. */
    if (!aptShouldClose())
        citadel_3ds_audio_service_resume();
#endif
    ...
    if (!citadel_v16_splash_started) {
        ...
        v5_log("GPU V16.1 SPLASH begin duration_ms=%u ready=%d",
    ...
    if (citadel_v16_splash_ready &&
        ...
        v5_log("GPU V16.1 SPLASH presentation failed; continuing to game");
        citadel_v16_splash_ready = false;
    }

    if (!citadel_v16_splash_finished_logged) {
        v5_log("GPU V16.1 SPLASH complete; beginning System Shock presentation");
