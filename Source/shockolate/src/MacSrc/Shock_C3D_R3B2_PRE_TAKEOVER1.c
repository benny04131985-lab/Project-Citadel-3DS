...
/* -------------------------------------------------------------------------
 * PROJECT CITADEL C3D R3B1 NATIVE STEREO ROUTE1
 *
 * R3A2 camera-space terrain remains unchanged. The engine's explicit
 * right/left render-pass boundaries now select the native capture buffer.
 *
 * BASE: PROJECT CITADEL C3D R3A2 WORLDSPACE SOLIDROOM1
 *
 * Shock supplies already-clipped camera-space terrain vertices. PICA200 owns
 * perspective projection, depth testing, and triangle rasterization.
 *
 * The complete R2C software frame remains visible underneath. This proof
 * intentionally does not suppress any software world surface yet.
 * ------------------------------------------------------------------------- */
#define CITADEL_NATIVE_WORLDPROOF_LOG \
    "sdmc:/3ds/SystemShock3D/C3D_R3B1_STEREOROUTE.log"
...
{
    citadel_native_worldproof_dvlb =
        DVLB_ParseFile(
            (u32 *)citadel_worldproof_vshader_shbin,
            citadel_worldproof_vshader_shbin_size);

    if (citadel_native_worldproof_dvlb == NULL) {
        v5_log("GPU C3D R3A2 WORLDSPACE INIT FAIL stage=DVLB_ParseFile");
        return false;
    }
    ...
    if (citadel_native_worldproof_projection_uniform < 0 ||
        ...
        v5_log("GPU C3D R3A2 WORLDSPACE INIT FAIL stage=uniforms "
               "projection=%d modelView=%d",
               citadel_native_worldproof_projection_uniform,
               citadel_native_worldproof_model_view_uniform);
    ...
    /*
     * Shock's OpenGL backend submits (gX, gY, -gZ) with equal X/Y
     * projection scale. A 1.0 aspect reproduces that camera-space mapping.
     */
    Mtx_PerspTilt(
        &citadel_native_worldproof_projection,
        C3D_AngleFromDegrees(89.5f),
        1.0f,
        0.01f,
        1000.0f,
        false);
    ...
    v5_log("GPU C3D R3A2 WORLDSPACE INIT SUCCESS "
           "shader_bytes=%u fov=89.5 aspect=1.0 near=0.01 far=1000",
           (unsigned int)citadel_worldproof_vshader_shbin_size);
...
static void citadel_native_worldproof_write_start(void)
{
    ...
    fprintf(file,
            "============================================================\n"
            "PROJECT CITADEL C3D R3B1 NATIVE STEREO ROUTE1 ACTIVE\n"
            "============================================================\n"
            "Build: %s %s\n"
            "Base geometry: CITADEL-C3D-R3A2-WORLDSPACE-SOLIDROOM1\n"
            "Mode: WORLDSPACE_TERRAIN_MIRROR_EXPLICIT_EYE_ROUTE\n"
            "Explicit engine eye routing: YES\n"
            "Software fallback: R2C FULLY PRESERVED\n"
            "Software polygon suppression: NO\n"
            "Native input: CLIPPED_CAMERA_SPACE_G3_XYZ\n"
            "Native surfaces: WALL FLOOR CEILING ONLY\n"
            "Native output: PERSPECTIVE_DEPTH_TESTED_CITRO3D_TRIANGLES\n"
            "Shader ready: %s\n"
            "Maximum triangles per eye: %u\n"
            "Draw budget per eye: %u\n"
            "Per-frame file writes: NO\n"
            "Shutdown summary pending: YES\n"
            "============================================================\n",
            __DATE__,
            __TIME__,
            citadel_native_worldproof_shader_ready ? "YES" : "NO",
...
static void citadel_native_worldproof_write_summary(void)
{
    ...
    FILE *file;
    bool right_route_proven;

    citadel_native_world_get_stats(&stats);

    right_route_proven =
        stats.stereo_engine_frames > 0 &&
        stats.right_eye_selects > 0 &&
        stats.right_capture_calls > 0 &&
        stats.right_triangles_captured > 0 &&
        stats.right_triangles_drawn > 0;
    ...
    fprintf(file,
            "\n==================== SESSION SUMMARY =======================\n"
            "Milestone: CITADEL-C3D-R3B1-NATIVE-STEREOROUTE1\n"
            "Base geometry: CITADEL-C3D-R3A2-WORLDSPACE-SOLIDROOM1\n"
            "Mode: WORLDSPACE_TERRAIN_MIRROR_EXPLICIT_EYE_ROUTE\n"
            "Shader ready: %s\n"
            "Software polygon suppression: NO\n"
            "Engine frames: %llu\n"
            "Mono engine frames: %llu\n"
            "Stereo engine frames: %llu\n"
            "Left eye selections: %llu\n"
            "Right eye selections: %llu\n"
            "Begin-view eye mismatches: %llu\n"
            "Left views captured: %u\n"
            "Right views captured: %u\n"
            "Left capture calls: %llu\n"
            "Right capture calls: %llu\n"
            "Left triangles captured: %llu\n"
            "Right triangles captured: %llu\n"
            "Wall triangles captured: %llu\n"
            "Floor triangles captured: %llu\n"
            "Ceiling triangles captured: %llu\n"
            "Generic textured calls skipped: %llu\n"
            "Solid polygon calls skipped: %llu\n"
            "Behind/near rejects: %llu\n"
            "Degenerate rejects: %llu\n"
            "Capture overflows: %llu\n"
            "Left triangles drawn: %llu\n"
            "Right triangles drawn: %llu\n"
            "Draw-budget drops: %llu\n"
            "Left last/peak triangles: %u / %u\n"
            "Right last/peak triangles: %u / %u\n"
            "Native right-eye route proven: %s\n"
            "Camera-space range valid: %s\n"
            "Camera-space X range: %.6f .. %.6f\n"
            "Camera-space Y range: %.6f .. %.6f\n"
            "Camera-space Z range: %.6f .. %.6f\n"
            "GPU upload failures: %u\n"
            "GPU draw failures: %u\n"
            "Clean Shutdown: YES\n"
            "============================================================\n",
            citadel_native_worldproof_shader_ready ? "YES" : "NO",
            (unsigned long long)stats.engine_frames,
            (unsigned long long)stats.mono_engine_frames,
            (unsigned long long)stats.stereo_engine_frames,
            (unsigned long long)stats.left_eye_selects,
            (unsigned long long)stats.right_eye_selects,
            (unsigned long long)stats.begin_view_eye_mismatches,
            stats.left_views,
            stats.right_views,
            (unsigned long long)stats.left_capture_calls,
            (unsigned long long)stats.right_capture_calls,
            (unsigned long long)stats.left_triangles_captured,
            (unsigned long long)stats.right_triangles_captured,
            (unsigned long long)stats.wall_triangles_captured,
            (unsigned long long)stats.floor_triangles_captured,
            (unsigned long long)stats.ceiling_triangles_captured,
            (unsigned long long)stats.generic_textured_calls_skipped,
            (unsigned long long)stats.solid_polygon_calls_skipped,
            (unsigned long long)stats.behind_or_near_rejects,
            (unsigned long long)stats.degenerate_rejects,
            (unsigned long long)stats.capture_overflows,
            (unsigned long long)stats.left_triangles_drawn,
            (unsigned long long)stats.right_triangles_drawn,
            (unsigned long long)stats.draw_budget_drops,
            stats.left_last_count,
            stats.left_peak_count,
            stats.right_last_count,
            stats.right_peak_count,
            right_route_proven ? "YES" : "NO",
            stats.range_valid ? "YES" : "NO",
            (double)stats.minimum_x,
            (double)stats.maximum_x,
            (double)stats.minimum_y,
            (double)stats.maximum_y,
            (double)stats.minimum_z,
            (double)stats.maximum_z,
            citadel_gpu_upload_failures,
            citadel_gpu_draw_failures);
...
static void citadel_native_worldproof_send_vertex(
    float x,
    float y,
    float z,
    uint8_t kind)
{
    ...
    /* Match Shockolate OpenGL: (gX, gY, -gZ) / 65536. */
    ...
    float destination_h)
{
    ...
    if (target == NULL ||
        !citadel_native_worldproof_shader_ready)
        return true;

    /*
     * Calibrate the first proof only in Dual/split mode, where the world
     * occupies the complete 400x240 top LCD. Legacy stays pure R2C.
     */
    ...
    /* Preserve R2C color, submit it, then clear only native depth. */
    C3D_FrameSplit(0);
    ...
    /* Mtx_PerspTilt uses reversed depth; depth zero is the far clear. */
    C3D_DepthTest(
        true,
        GPU_GREATER,
        GPU_WRITE_ALL);
    ...
    for (index = 0; index < draw_count; ++index) {
        const CitadelNativeWorldTriangle *triangle =
            &triangles[index];

        citadel_native_worldproof_send_vertex(
            triangle->x0,
            triangle->y0,
            triangle->z0,
            triangle->kind);
...

    {
        ...
        if (!*first_draw_logged) {
            v5_log("GPU C3D R3B1 NATIVE STEREO FIRST DRAW "
                   "eye=%s captured=%u drawn=%u dropped=%u "
                   "projection=PERSPECTIVE depth=GREATER "
                   "surfaces=WALL_FLOOR_CEILING",
...
static bool citadel_gpu_initialize(void)
{
    ...
    v5_log("GPU C3D R3B1 NATIVE STEREO ROUTE1 ACTIVE "
           "mode=WORLDSPACE_TERRAIN_MIRROR_EXPLICIT_EYE_ROUTE "
           "shader_ready=%u software_suppression=0 "
           "surfaces=WALL_FLOOR_CEILING "
           "capture_max=%u draw_budget=%u",
