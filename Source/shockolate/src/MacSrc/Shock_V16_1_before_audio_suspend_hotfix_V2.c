...
#warning "PROJECT CITADEL AUDIO SUSPEND HOTFIX: libctru APT hook is ACTIVE"
    ...
    void *parameter)
{
    ...
    switch (hook) {
        ...
        case APTHOOK_ONWAKEUP:
            /*
             * Closing Software from HOME can wake the process only so it
             * can finish teardown. Do not restart the audio callback when
             * APT has already ordered the application to close.
             */
            if (!aptShouldClose())
