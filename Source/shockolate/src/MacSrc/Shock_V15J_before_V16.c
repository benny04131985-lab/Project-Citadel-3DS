...
static bool citadel_gpu_initialize(void)
{
    ...
    v5_log("GPU V15J SCREEN FORMAT before-targets "
    ...
    v5_log("GPU V15J TARGET OUTPUT transfer_in=RGBA8 "
...
static unsigned int citadel_gpu_refresh_palette565(void)
{
    ...
    if (citadel_gpu_palette_refreshes == 1 ||
        ...
        v5_log("GPU V15J PALETTE SYNC refresh=%lu mismatches=%u "
                                         ...
                                         C3D_Tex *texture)
{
    ...
    if (texture == NULL) {
        v5_log("GPU V15J TEX %s NULL",
               label != NULL ? label : "(null)");
        return;
    }

    v5_log("GPU V15J TEX %s data=%p phys=0x%08lX "
                                                     ...
                                                     uint32_t linear_hash)
{
    ...
    v5_log("GPU V15J LOAD VERIFY rgb565_memcmp=%d "
    ...
    v5_log("GPU V15J PHASE begin=%s duration_ms=6000 "
...
static bool citadel_gpu_v15h_load_sheet(void)
{
    ...
    for (i = 0; i < sizeof(paths) / sizeof(paths[0]); ++i) {
        ...
        if (citadel_gpu_v15h_sheet != NULL) {
            v5_log("GPU V15J TEX3DS LOAD path=%s result=SUCCESS count=%lu",
                   paths[i],
        ...
        v5_log("GPU V15J TEX3DS LOAD path=%s result=FAIL",
               paths[i]);
    }
...
static bool citadel_gpu_v15h_initialize_control(void)
{
    ...
    if (C2D_SpriteSheetCount(citadel_gpu_v15h_sheet) < 1) {
        v5_log("GPU V15J TEX3DS LOAD FAIL reason=no-images");
        return false;
    }
    ...
    if (citadel_gpu_v15h_official_image.tex == NULL ||
        ...
        v5_log("GPU V15J TEX3DS LOAD FAIL reason=invalid-image");
        return false;
    }
    ...
    v5_log("GPU V15J SUBTEX official width=%u height=%u "
    ...
    if (citadel_gpu_v15h_official_image.tex->width !=
        ...
        v5_log("GPU V15J TEX3DS LOAD FAIL "
    ...
    if (!C3D_TexInit(&citadel_gpu_v15h_manual_texture,
                     ...
                     CITADEL_V15H_CONTROL_SIZE,
                     GPU_RGB565)) {
        v5_log("GPU V15J MANUAL INIT FAIL stage=C3D_TexInit");
        return false;
    }
    ...
    if (citadel_gpu_v15h_candidate == NULL) {
        v5_log("GPU V15J MANUAL INIT FAIL stage=linearAlloc bytes=%u",
    ...
    for (variant = 0; variant <= 5; ++variant) {
        ...
        v5_log("GPU V15J GOLDEN COMPARE variant=%d "
...
static bool citadel_gpu_upload_surface(SDL_Surface *surface)
{
    ...
    if (citadel_gpu_v15h_phase ==
        ...
        v5_log("GPU V15J PHASE complete; "
    ...
    if (!citadel_gpu_v15h_capture_done &&
        ...
        if (citadel_gpu_v15h_asset_ready) {
            ...
            citadel_gpu_v15h_phase =
                CITADEL_V15H_PHASE_COMPLETE;
            v5_log("GPU V15J PHASE SKIPPED reason=asset-unavailable");
        }
    }

    if (!citadel_gpu_first_upload_logged) {
        v5_log("GPU V15J FIRST LIVE FEED src=%dx%d pitch=%d "
...
static bool citadel_gpu_present(SDL_Surface *surface, bool magenta)
{
    ...
    if (!magenta && !citadel_gpu_sync_order_logged) {
        v5_log("GPU V15J SYNC ORDER confirmed: "
    ...
    if (!magenta &&
        ...
        if (!citadel_gpu_cache_rebind_logged) {
            v5_log("GPU V15J CACHE REBIND active: "
...
static void v5_log(const char *fmt, ...)
{
    FILE *file = fopen("GPU_C2D_V15J.log", "a");
    va_list args;
...
void InitSDL() {
    ...
    remove("GPU_C2D_V15J.log");
    v5_log("PROJECT CITADEL V15J FULL 640x480 TEXTURE TRANSPORT START | build=%s %s", __DATE__, __TIME__);
