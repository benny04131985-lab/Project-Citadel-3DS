...
#warning "PROJECT CITADEL HOME FRAME-GATE TEST 4: APT-first freeze wrapper is ACTIVE"
 ...
 *
 * The APT hook marks the application frozen from the instant HOME/sleep
 * suspension begins until libctru reports restore/wakeup. mainloop.c uses
 * these flags to prevent timers, input, simulation, and rendering from
 * advancing during that transition.
 */
...
/*
 * Complete all previously submitted Citro3D work before SDL/libctru services
 * APT at the beginning of the next Shock frame. This leaves no outstanding
 * PICA200 frame while HOME Menu takes ownership of the displays.
 */
void citadel_3ds_prepare_gpu_for_apt(void)
{
