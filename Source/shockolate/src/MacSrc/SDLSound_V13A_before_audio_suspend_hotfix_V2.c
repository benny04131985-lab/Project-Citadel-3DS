...
#warning "PROJECT CITADEL AUDIO SUSPEND HOTFIX: APT pause/resume is ACTIVE"
...
/*
 * HOME Menu owns the DSP while the application is suspended. Keep the
 * existing SDL device and all mixer/music state alive, but stop its callback
 * before libctru performs the APT DSP handoff.
 */
static bool citadel_v16_1_audio_suspended = false;
...
/*
 * Called from Project Citadel's libctru APT hook.
 *
 * SDL_PauseAudioDevice() waits until the active callback is no longer
 * running and guarantees that it will not be called while the device is
 * paused. That is the exact boundary needed before HOME Menu takes DSP
 * ownership.
 */
void citadel_3ds_audio_suspend(void)
{
    SDL_AudioStatus status_before;

    if (!citadel_v12c_audio_ready ||
        device == 0 ||
        citadel_v16_1_audio_suspended) {
        return;
    }

    status_before = SDL_GetAudioDeviceStatus(device);
    SDL_PauseAudioDevice(device, 1);
    citadel_v16_1_audio_suspended = true;

    citadel_v12c_log(
        "APT AUDIO SUSPEND device=%u before=%d after=%d "
        "music_ready=%d",
        (unsigned int)device,
        (int)status_before,
...
void citadel_3ds_audio_resume(void)
{
    ...
    citadel_v16_1_audio_suspended = false;

    citadel_v12c_log(
        "APT AUDIO RESUME device=%u before=%d after=%d "
        "music_ready=%d",
        (unsigned int)device,
        (int)status_before,
        (int)SDL_GetAudioDeviceStatus(device),
...
static int citadel_v12c_open_audio(void)
{
    ...
    citadel_v12c_log(
        "PROJECT CITADEL AUDIO V13A START build=%s %s "
        "voices=%d",
        __DATE__,
        __TIME__,
        SND_MAX_SAMPLES);
