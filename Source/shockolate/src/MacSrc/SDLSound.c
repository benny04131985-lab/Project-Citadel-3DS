...
#warning "PROJECT CITADEL SDLSOUND V13A: DIGIFX speech and native OPL music are ACTIVE"
#warning "PROJECT CITADEL AUDIO SUSPEND HOTFIX V2: NDSP close/deferred reopen is ACTIVE"
...
#define CITADEL_V12D_SPEECH_CHANNEL (SND_MAX_SAMPLES - 1)
...
static bool citadel_v13a_music_attempted = false;
static bool citadel_v13a_music_ready = false;
static bool citadel_v13a_audio_shutdown_registered = false;
/*
 * SDL's 3DS backend keeps NDSP channel/wave-buffer state alive while merely
 * paused. HOME therefore closes only the SDL output device. Project Citadel's
 * voices, stream, XMI sequencer and ADLMIDI state remain allocated.
 */
static bool citadel_v16_2_audio_suspended = false;
static bool citadel_v16_2_audio_resume_pending = false;
static bool citadel_v16_2_audio_initialized_once = false;
static volatile Uint32 citadel_v13a_music_callbacks = 0;
static volatile Uint32 citadel_v13a_music_bytes = 0;
static volatile Uint32 citadel_v13a_music_nonzero_buffers = 0;
...
static void citadel_v12c_log(const char *fmt, ...)
{
    ...
    file = fopen("AUDIO_3DS_V13A.log", "a");
    if (file == NULL)
        return;
...
/*
 * APTHOOK_ONSUSPEND runs before libctru calls aptDspSleep(). Closing the SDL
 * device makes SDL's N3DS backend reset NDSP channel 0, clear wave buffers,
 * unregister its DSP hook and call ndspExit(). Do not use the normal Project
 * Citadel shutdown routine here: it also frees the preserved voices.
 */
void citadel_3ds_audio_suspend(void)
{
    SDL_AudioDeviceID closing_device;

    citadel_v16_2_audio_resume_pending = false;

    if (citadel_v16_2_audio_suspended)
        return;

    if (!citadel_v12c_audio_ready || device == 0) {
        citadel_v16_2_audio_suspended = true;
        citadel_v12c_log(
            "APT AUDIO CLOSE skipped ready=%d device=%u",
            citadel_v12c_audio_ready ? 1 : 0,
            (unsigned int)device);
        return;
    }
    ...
    citadel_v12c_log(
        "APT AUDIO CLOSE BEGIN device=%u status=%d music_ready=%d",
        (unsigned int)closing_device,
        (int)SDL_GetAudioDeviceStatus(closing_device),
        citadel_v13a_music_ready ? 1 : 0);

    SDL_CloseAudioDevice(closing_device);

    device = 0;
    citadel_v12c_audio_ready = false;
    citadel_v12c_audio_attempted = false;
    citadel_v16_2_audio_suspended = true;

    citadel_v12c_log(
        "APT AUDIO CLOSE COMPLETE old_device=%u voices_preserved=1 music_ready=%d",
        (unsigned int)closing_device,
        citadel_v13a_music_ready ? 1 : 0);
}

/*
 * HOME's APTHOOK_ONRESTORE occurs before aptDspWakeup(). Mark the request
 * here; Shock.c reopens from SDLDraw() after the APT transition returns.
 */
void citadel_3ds_audio_resume(void)
{
    if (!citadel_v16_2_audio_suspended)
        return;

    citadel_v16_2_audio_resume_pending = true;
    citadel_v12c_log(
        "APT AUDIO REOPEN DEFERRED ready=%d device=%u music_ready=%d",
        citadel_v12c_audio_ready ? 1 : 0,
        (unsigned int)device,
        citadel_v13a_music_ready ? 1 : 0);
}

void citadel_3ds_audio_service_resume(void)
{
    int result;

    if (!citadel_v16_2_audio_resume_pending)
        return;

    if (citadel_v12c_audio_ready && device != 0) {
        citadel_v16_2_audio_resume_pending = false;
        citadel_v16_2_audio_suspended = false;
        return;
    }

    citadel_v16_2_audio_resume_pending = false;
    citadel_v12c_log(
        "APT AUDIO REOPEN BEGIN voices_preserved=1 music_ready=%d",
        citadel_v13a_music_ready ? 1 : 0);

    result = citadel_v12c_open_audio();

    if (result == OK && citadel_v12c_audio_ready && device != 0) {
        citadel_v16_2_audio_suspended = false;
        citadel_v12c_log(
            "APT AUDIO REOPEN COMPLETE device=%u status=%d music_ready=%d",
            (unsigned int)device,
        ...
        citadel_v16_2_audio_suspended = true;
        citadel_v12c_log(
            "APT AUDIO REOPEN FAILED result=%d device=%u error=\"%s\"",
            result,
            (unsigned int)device,
            SDL_GetError());
    }
    ...
    int len)
{
    ...
    /*
     * The XMI worker thread feeds MIDI events into MusicDev. Generate the
     * corresponding OPL PCM here and add it to the same device buffer as
     * DIGIFX and the cutscene/audio-log stream.
     */
    if (citadel_v13a_music_ready && MusicDev != NULL) {
        Uint8 music_chunk[2048];
        int mixed_bytes = 0;
        ...
        while (mixed_bytes < len) {
            ...
            if (requested > (int)sizeof(music_chunk))
                requested = (int)sizeof(music_chunk);

            requested &= ~3;
            if (requested <= 0)
                break;

            SDL_memset(music_chunk, 0, (size_t)requested);
            MusicCallback((void *)&MusicDev, music_chunk, requested);

            for (index = 0; index < requested; ++index) {
                if (music_chunk[index] != 0) {
                    any_nonzero = true;
                    break;
                }
            }
            ...
            mixed_bytes += requested;
            citadel_v13a_music_bytes += (Uint32)requested;
        }

        if (mixed_bytes > 0)
            ++citadel_v13a_music_callbacks;

        if (any_nonzero)
            ++citadel_v13a_music_nonzero_buffers;
    }
...
static void citadel_v12c_audio_shutdown(void)
{
    ...
    citadel_v13a_music_ready = false;
    ...
    citadel_v12c_log(
        "MUSIC SUMMARY attempted=%d ready=%d callbacks=%u "
        "bytes=%u nonzero_buffers=%u",
        citadel_v13a_music_attempted ? 1 : 0,
        citadel_v13a_music_ready ? 1 : 0,
        (unsigned int)citadel_v13a_music_callbacks,
        (unsigned int)citadel_v13a_music_bytes,
        (unsigned int)citadel_v13a_music_nonzero_buffers);
...
static int citadel_v12c_open_audio(void)
{
    ...
    first_open = !citadel_v16_2_audio_initialized_once;
    citadel_v12c_audio_attempted = true;

    if (first_open) {
        remove("AUDIO_3DS_V13A.log");
        ...
        citadel_v12c_log(
            "PROJECT CITADEL AUDIO V13A+SUSPENDV2 START build=%s %s voices=%d",
            __DATE__,
            __TIME__,
            SND_MAX_SAMPLES);
    } else {
        citadel_v12c_log(
            "APT AUDIO OPEN REQUEST preserving voices/music");
    }
    ...
    citadel_v16_2_audio_suspended = false;
    citadel_v16_2_audio_resume_pending = false;
    citadel_v16_2_audio_initialized_once = true;
...
static int citadel_v12c_choose_channel_locked(int priority)
{
    ...
    for (channel = 0; channel < CITADEL_V12D_SPEECH_CHANNEL; ++channel) {
        if (!citadel_v12c_voices[channel].active
...
int snd_start_digital(void)
{
    int result = citadel_v12c_open_audio();

    if (result != OK)
        return result;

    if (!citadel_v13a_music_attempted) {
        citadel_v13a_music_attempted = true;
        InitReadXMI();

        citadel_v13a_music_ready =
            MusicDev != NULL && MusicDev->isOpen;

        citadel_v12c_log(
            "MUSIC INIT ready=%d device=%p type=%d "
            "rate=%d mode=%s",
            citadel_v13a_music_ready ? 1 : 0,
            (void *)MusicDev,
            MusicDev != NULL ? (int)MusicDev->deviceType : -1,
            citadel_3ds_audio_output_rate(),
            MusicDev != NULL && MusicDev->musicType != NULL
                ? MusicDev->musicType
                : "(none)");
    }

    if (!citadel_v13a_audio_shutdown_registered) {
        atexit(citadel_v12c_audio_shutdown);
        citadel_v13a_audio_shutdown_registered = true;
    }
...
#define KEY_BAR_RESOLUTION 2
    ...
    extern uchar transition_table[NUM_TRANSITIONS];
    extern uchar layering_table[NUM_LAYERS][MAX_KEYS];
    extern uchar key_table[
        NUM_LAYERABLE_SUPERCHUNKS][KEY_BAR_RESOLUTION];
    extern FILE *fopen_caseless(
        const char *path,
        const char *mode);

    if (!citadel_v13a_music_ready
        || MusicDev == NULL
        || !MusicDev->isOpen
        || theme_base == NULL) {
        citadel_v12c_log(
            "MUSIC THEME FAIL base=%s id=%d reason=not-ready",
            theme_base != NULL ? theme_base : "(null)",
            themeID);
        return ERR_NOEFFECT;
    }
    ...
    if (strncmp(theme_base, "thm", 3) != 0) {
        ...
        snprintf(
            filename,
            sizeof(filename),
            "res/sound/%s/thm%d.xmi",
            MusicDev->musicType,
            themeID);
    }

    read_result = ReadXMI(filename);

    citadel_v12c_log(
        "MUSIC THEME base=%s id=%d path=%s read=%d tracks=%u",
        theme_base,
        themeID,
        filename,
        read_result,
    ...
    if (strncmp(theme_base, "thm", 3) == 0) {
        ...
        file = fopen_caseless(filename, "rb");
        if (file != NULL) {
            blocks_read += fread(
                track_table,
            ...
            citadel_v12c_log(
                "MUSIC TABLE path=%s blocks=%u",
                filename,
                (unsigned int)blocks_read);
        } else {
            citadel_v12c_log(
                "MUSIC TABLE MISSING path=%s",
                filename);
        }
    }
