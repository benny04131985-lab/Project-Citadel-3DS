...
static bool citadel_gpu_initialize(void)
{
    ...
    v5_log("GPU V15A DIRECT TILED ready texture=%dx%d content=%dx%d "
...
static void v5_log(const char *fmt, ...)
{
    FILE *file = fopen("GPU_C2D_V15A.log", "a");
    va_list args;
...
void InitSDL() {
    ...
    remove("GPU_C2D_V15A.log");
    v5_log("PROJECT CITADEL V15A DIRECT MORTON LIVE UPLOAD START | build=%s %s", __DATE__, __TIME__);
