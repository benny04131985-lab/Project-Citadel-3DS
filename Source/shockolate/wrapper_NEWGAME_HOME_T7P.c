...
#warning "PROJECT CITADEL NEW GAME HOME NORMALIZER T7P is ACTIVE"

/*
 * T7 reproduces the exact successful manual lifecycle demonstrated on hardware:
 *
 *   fresh New Game
 *     -> open the real numbered-slot Save screen
 *     -> leave it installed and visible for one complete GAME_LOOP frame
 *     -> invoke the genuine save callback before the next game frame renders
 *     -> render one complete gameplay frame
 *     -> open the real numbered-slot Load screen
 *     -> leave it installed and visible for one complete GAME_LOOP frame
 *     -> invoke the genuine text-list selection/load callback before rendering
 *     -> render the loaded gameplay state
 *     -> delete the temporary numbered save and restore its metadata
 *
 * Unlike earlier attempts, this uses Poke_SaveName() indirectly through the original
 * numbered-slot callbacks, keeps each wrapper alive across a real frame, and
 * performs the callback at the same pre-render point as ordinary UI input.
 * Continue and every user-selected save/load path are untouched.
 */
typedef enum Citadel3dsT7Stage {
    CITADEL_T7_IDLE = 0,
    CITADEL_T7_ARMED,
    CITADEL_T7_SAVE_WRAPPER_PRESENTING,
    CITADEL_T7_SAVE_CALLBACK_PENDING,
    CITADEL_T7_SAVE_DONE_RENDER_GAME,
    CITADEL_T7_LOAD_WRAPPER_PRESENTING,
    CITADEL_T7_LOAD_CALLBACK_PENDING,
    CITADEL_T7_LOAD_DONE_RENDER_GAME,
    CITADEL_T7_COMPLETE,
    CITADEL_T7_FAILED
} Citadel3dsT7Stage;

static Citadel3dsT7Stage citadel_3ds_t7_stage = CITADEL_T7_IDLE;
static int citadel_3ds_t7_slot = -1;
static bool citadel_3ds_t7_temp_created = false;
static char citadel_3ds_t7_original_comment[SAVE_COMMENT_LEN];
static const char citadel_3ds_t7_temp_comment[] = "3DS New Game Init";

static void citadel_3ds_t7_log(const char *message)
{
    FILE *file = fopen("NEWGAME_HOME_T7.log", "a");
...
static void citadel_3ds_t7_log_state(const char *message)
{
    extern uchar valid_save;
    FILE *file = fopen("NEWGAME_HOME_T7.log", "a");
    ...
    fprintf(file,
            "%s | stage=%d slot=%d wrapper=%u valid=0x%02X file=%s\n",
            message,
            (int)citadel_3ds_t7_stage,
            citadel_3ds_t7_slot,
            (unsigned int)wrapper_panel_on,
            (unsigned int)valid_save,
            save_game_name);
    fflush(file);
    fclose(file);
}

static bool citadel_3ds_t7_slot_file_exists(int slot)
{
    FILE *file;

    Poke_SaveName((uchar)slot);
    file = fopen(save_game_name, "rb");
    if (file == NULL)
        return false;
...
static int citadel_3ds_t7_find_free_slot(void)
{
    extern uchar valid_save;
    int slot;

    /*
     * Prefer the highest genuinely empty visible slot. A cleared valid_save bit
     * is not enough: an old/bad-version file may still exist on disk, and T7
     * must never delete or overwrite any pre-existing file.
     */
    for (slot = NUM_SAVE_SLOTS - 1; slot >= 0; --slot) {
        if (!(valid_save & (1U << slot)) &&
            !citadel_3ds_t7_slot_file_exists(slot))
            return slot;
    }
...
static void citadel_3ds_t7_restore_temp_slot(void)
{
    extern uchar valid_save;

    if (citadel_3ds_t7_slot < 0 ||
        citadel_3ds_t7_slot >= NUM_SAVE_SLOTS)
        return;

    Poke_SaveName((uchar)citadel_3ds_t7_slot);
    if (citadel_3ds_t7_temp_created)
        remove(save_game_name);
    citadel_3ds_t7_temp_created = false;

    valid_save &= (uchar)~(1U << citadel_3ds_t7_slot);
    strncpy(comments[citadel_3ds_t7_slot],
            citadel_3ds_t7_original_comment,
            SAVE_COMMENT_LEN - 1);
    comments[citadel_3ds_t7_slot][SAVE_COMMENT_LEN - 1] = '\0';
}

static void citadel_3ds_t7_fail(const char *message)
{
    if (wrapper_panel_on)
        wrapper_panel_close(TRUE);

    citadel_3ds_t7_restore_temp_slot();
    citadel_3ds_t7_stage = CITADEL_T7_FAILED;
    citadel_3ds_t7_log_state(message);
}

bool citadel_3ds_newgame_home_t7_mask_active(void)
{
    /*
     * Do not mask the tutorial or the first usable gameplay frame. The mask
     * begins only once T7 has actually opened its hidden Save wrapper, and it
     * remains active through the loaded gameplay frame that proves the
     * normalization completed.
     */
    return citadel_3ds_t7_stage >= CITADEL_T7_SAVE_WRAPPER_PRESENTING &&
           citadel_3ds_t7_stage <= CITADEL_T7_LOAD_DONE_RENDER_GAME;
}

unsigned int citadel_3ds_newgame_home_t7_mask_step(void)
{
    switch (citadel_3ds_t7_stage) {
    case CITADEL_T7_SAVE_WRAPPER_PRESENTING:
        return 1U;
    case CITADEL_T7_SAVE_CALLBACK_PENDING:
        return 2U;
    case CITADEL_T7_SAVE_DONE_RENDER_GAME:
        return 3U;
    case CITADEL_T7_LOAD_WRAPPER_PRESENTING:
        return 4U;
    case CITADEL_T7_LOAD_CALLBACK_PENDING:
        return 5U;
    case CITADEL_T7_LOAD_DONE_RENDER_GAME:
        return 6U;
    default:
        return 0U;
    }
...
{
    remove("NEWGAME_HOME_T7.log");

    citadel_3ds_t7_slot = -1;
    citadel_3ds_t7_temp_created = false;
    citadel_3ds_t7_original_comment[0] = '\0';
    citadel_3ds_t7_stage = CITADEL_T7_ARMED;

    citadel_3ds_t7_log("T7 BUILD: " __DATE__ " " __TIME__);
    citadel_3ds_t7_log_state(
        "T7 ARMED: finalized New Game; waiting for a complete GAME_LOOP frame");
}

void citadel_3ds_newgame_home_t7_pre_frame(void)
{
    extern uchar valid_save;
    opt_textlist_state *load_list;

    if (citadel_3ds_t7_stage == CITADEL_T7_SAVE_CALLBACK_PENDING) {
        if (!wrapper_panel_on) {
            citadel_3ds_t7_fail(
                "T7 FAILED: Save wrapper disappeared before its callback");
            return;
        }

        citadel_3ds_t7_log_state(
            "T7 PRE-FRAME: invoking genuine numbered-slot Save callback");

        save_dealfunc(0, (uchar)citadel_3ds_t7_slot);

        if (wrapper_panel_on ||
            !(valid_save & (1U << citadel_3ds_t7_slot))) {
            citadel_3ds_t7_fail(
                "T7 FAILED: genuine Save callback did not complete normally");
            return;
        }

        citadel_3ds_t7_temp_created = true;
        citadel_3ds_t7_stage = CITADEL_T7_SAVE_DONE_RENDER_GAME;
        citadel_3ds_t7_log_state(
            "T7 SAVE COMPLETE: numbered slot saved and wrapper closed before render");
        return;
    }

    if (citadel_3ds_t7_stage == CITADEL_T7_LOAD_CALLBACK_PENDING) {
        if (!wrapper_panel_on) {
            citadel_3ds_t7_fail(
                "T7 FAILED: Load wrapper disappeared before its callback");
            return;
        }

        /*
         * Enter through the actual text-list selection path used by a real slot
         * click/Enter. It redraws the selection and invokes load_dealfunc(), which
         * performs Poke_SaveName(), begin_wait(), load_game(), end_wait(), and the
         * normal wrapper_panel_close() sequence.
         */
        load_list = &OButtons[0].user.textlist_st;
        citadel_3ds_t7_last_load_result = ERR_NOEFFECT;
        citadel_3ds_t7_log_state(
            "T7 PRE-FRAME: invoking genuine Load text-list selection callback");
        textlist_select_line(load_list,
                             0,
                             (uchar)citadel_3ds_t7_slot,
                             TRUE);

        if (wrapper_panel_on) {
            citadel_3ds_t7_fail(
                "T7 FAILED: genuine Load callback left wrapper open");
            return;
        }

        if (citadel_3ds_t7_last_load_result != OK) {
            citadel_3ds_t7_fail(
                "T7 FAILED: genuine numbered-slot load_game returned an error");
            return;
        }

        citadel_3ds_t7_stage = CITADEL_T7_LOAD_DONE_RENDER_GAME;
        citadel_3ds_t7_log_state(
            "T7 LOAD CALLBACK COMPLETE: loaded state will render this frame");
    }
}
...
void citadel_3ds_newgame_home_t7_post_frame(void)
{
    ...
    switch (citadel_3ds_t7_stage) {
    case CITADEL_T7_ARMED:
        citadel_3ds_t7_slot = citadel_3ds_t7_find_free_slot();
        if (citadel_3ds_t7_slot < 0) {
            citadel_3ds_t7_fail(
                "T7 FAILED: all numbered save slots are occupied; no user save was touched");
            return;
        }

        strncpy(citadel_3ds_t7_original_comment,
                comments[citadel_3ds_t7_slot],
                SAVE_COMMENT_LEN - 1);
        citadel_3ds_t7_original_comment[SAVE_COMMENT_LEN - 1] = '\0';

        strncpy(comments[citadel_3ds_t7_slot],
                citadel_3ds_t7_temp_comment,
                SAVE_COMMENT_LEN - 1);
        comments[citadel_3ds_t7_slot][SAVE_COMMENT_LEN - 1] = '\0';
        ...
        citadel_3ds_t7_log_state(
            "T7 FRAME COMPLETE: opening genuine numbered-slot Save wrapper");
        wrapper_start(save_screen_init);
        citadel_3ds_t7_stage = CITADEL_T7_SAVE_WRAPPER_PRESENTING;
        return;

    case CITADEL_T7_SAVE_WRAPPER_PRESENTING:
        if (!wrapper_panel_on) {
            citadel_3ds_t7_fail(
                "T7 FAILED: Save wrapper was not alive for its presentation frame");
            return;
        }

        citadel_3ds_t7_stage = CITADEL_T7_SAVE_CALLBACK_PENDING;
        citadel_3ds_t7_log_state(
            "T7 SAVE WRAPPER PRESENTED: callback queued for next pre-frame");
        return;
    ...
    case CITADEL_T7_SAVE_DONE_RENDER_GAME:
        if (wrapper_panel_on ||
            !(valid_save & (1U << citadel_3ds_t7_slot))) {
            citadel_3ds_t7_fail(
                "T7 FAILED: saved slot state invalid before opening Load wrapper");
            return;
        }

        citadel_3ds_t7_log_state(
            "T7 GAME FRAME COMPLETE: opening genuine numbered-slot Load wrapper");
        wrapper_start(load_screen_init);
        citadel_3ds_t7_stage = CITADEL_T7_LOAD_WRAPPER_PRESENTING;
        return;

    case CITADEL_T7_LOAD_WRAPPER_PRESENTING:
        if (!wrapper_panel_on) {
            citadel_3ds_t7_fail(
                "T7 FAILED: Load wrapper was not alive for its presentation frame");
            return;
        }

        citadel_3ds_t7_stage = CITADEL_T7_LOAD_CALLBACK_PENDING;
        citadel_3ds_t7_log_state(
            "T7 LOAD WRAPPER PRESENTED: callback queued for next pre-frame");
        return;

    case CITADEL_T7_LOAD_DONE_RENDER_GAME:
        /*
         * The loaded state has now completed a normal GAME_LOOP render. Remove
         * only T7's free-slot file and restore the exact pre-T7 metadata.
         */
        citadel_3ds_t7_restore_temp_slot();
        citadel_3ds_t7_stage = CITADEL_T7_COMPLETE;
        citadel_3ds_t7_log_state(
            "T7 COMPLETE: exact numbered Save/Load lifecycle finished and temp slot removed");
        return;
