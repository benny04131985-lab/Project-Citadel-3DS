...
#warning "PROJECT CITADEL V15G: fresh texture binding test is ACTIVE"
...
static bool citadel_gpu_initialize(void)
{
    ...
    v5_log("GPU V15G FRESH TEXTURE BINDING ready texture=%dx%d content=%dx%d "
...
static bool citadel_gpu_present(SDL_Surface *surface, bool magenta)
{
     ...
     */
    if (!magenta && !citadel_gpu_v15g_phase_active()) {
...
static void v5_log(const char *fmt, ...)
{
    FILE *file = fopen("GPU_C2D_V15G.log", "a");
    va_list args;
...
void InitSDL() {
    ...
    remove("GPU_C2D_V15G.log");
    v5_log("PROJECT CITADEL V15G FRESH TEXTURE BINDING START | build=%s %s", __DATE__, __TIME__);
