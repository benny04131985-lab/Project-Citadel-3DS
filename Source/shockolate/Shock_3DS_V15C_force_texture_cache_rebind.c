...
static bool citadel_gpu_initialize(void)
{
    ...
    v5_log("GPU V15C CACHE-INVALIDATED DIRECT TILED ready texture=%dx%d content=%dx%d "
...
static bool citadel_gpu_upload_surface(SDL_Surface *surface)
{
    ...
    if (!citadel_gpu_first_upload_logged) {
        v5_log("GPU V15C FIRST DIRECT UPLOAD src=%dx%d pitch=%d "
...
static void v5_log(const char *fmt, ...)
{
    FILE *file = fopen("GPU_C2D_V15C.log", "a");
    va_list args;
...
void InitSDL() {
    ...
    remove("GPU_C2D_V15C.log");
    v5_log("PROJECT CITADEL V15C FORCED TEXTURE-CACHE REBIND START | build=%s %s", __DATE__, __TIME__);
