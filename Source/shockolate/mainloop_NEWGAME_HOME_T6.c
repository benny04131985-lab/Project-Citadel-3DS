...
#warning "PROJECT CITADEL T6: post-GAME_LOOP normalizer hook is ACTIVE"
...
#if defined(__3DS__) || defined(_3DS)
        /*
         * Do not count the SETUP_LOOP frame that merely switches modes.
         * T6 advances only after a frame that both began and ended in the
         * actual GAME_LOOP, after game logic and SDLDraw have completed.
         */
        if (loop_at_frame_start == GAME_LOOP &&
            _current_loop == GAME_LOOP &&
            gPlayingGame) {
            citadel_3ds_newgame_home_t6_tick();
        }
#endif
