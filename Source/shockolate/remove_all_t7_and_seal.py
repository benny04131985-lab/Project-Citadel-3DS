#!/usr/bin/env python3
"""
Project Citadel: remove T7 completely from the current VX1-NATURAL baseline.
...
def find_remnants(
    ...
    for path, text in staged.items():
        for line_number, line in enumerate(text.splitlines(), start=1):
            for pattern in T7_REMNANT_PATTERNS:
                if pattern.lower() in line.lower():
                    findings.append(
                        f"{path}:{line_number}: matched {pattern!r}: "
                        f"{line.strip()}"
                    )
...
def main() -> int:
    ...
    print("============================================================")
    print("PROJECT CITADEL VX1-NATURAL: REMOVE ALL T7")
    print("============================================================")
    print()
