...
static bool citadel_gpu_initialize(void)
{
    ...
    v5_log("GPU V15E AUTHORITATIVE PALETTE ready texture=%dx%d content=%dx%d "
...
static bool citadel_gpu_present(SDL_Surface *surface, bool magenta)
{
    ...
    if (!magenta && !citadel_gpu_sync_order_logged) {
        v5_log("GPU V15E SYNC ORDER confirmed: "
    ...
    if (!magenta) {
        ...
        if (!citadel_gpu_cache_rebind_logged) {
            v5_log("GPU V15E CACHE REBIND active: "
...
static void v5_log(const char *fmt, ...)
{
    FILE *file = fopen("GPU_C2D_V15E.log", "a");
    va_list args;
...
void InitSDL() {
    ...
    remove("GPU_C2D_V15E.log");
    v5_log("PROJECT CITADEL V15E AUTHORITATIVE PALETTE SYNC START | build=%s %s", __DATE__, __TIME__);
